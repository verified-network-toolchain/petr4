...
control pipe(inout Headers_t headers, inout metadata meta, inout standard_metadata std_meta) {
    @name("RejectConditional") action RejectConditional_0(@name("condition") bit<1> condition_0) {
    }
    @name("RejectConditional") action RejectConditional(@name("condition") bit<1> condition_0) {
    }
    ...
    @name("act_return") action act_return() {
        bool hasReturned = false;
        mark_to_pass();
        {
            hasReturned = true;

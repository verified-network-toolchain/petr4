/petr4/ci-test/type-checking/testdata/p4_16_samples/issue1025-bmv2.p4
\n
/*
...
parser parserI(packet_in pkt, out headers hdr, inout metadata meta,
  ...
  state parse_ipv4
    {
    pkt.extract(hdr.ipv4,
                  (bit<32>) 8*(bit<9>) pkt.lookahead<IPv4_up_to_ihl_only_h>().ihl);
    transition select(hdr.ipv4.protocol) {
      6: parse_tcp;
      default: accept;
    }
...
control cIngress(inout headers hdr, inout metadata meta,
  ...
  apply
    ...
    24w0 ++ 7w0 ++ eth_valid ++ 7w0 ++ ipv4_valid ++ 7w0 ++ tcp_valid;
  }
}
...
V1Switch(parserI(), verifyChecksum(), cIngress(), cEgress(),
           updateChecksum(), DeparserI())
  main;

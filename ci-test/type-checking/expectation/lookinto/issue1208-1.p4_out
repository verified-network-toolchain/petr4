/petr4/ci-test/type-checking/testdata/p4_16_samples/issue1208-1.p4
\n
#include <core.p4>
#include <bmv2/psa.p4>
struct EMPTY { };
...
parser MyIP(packet_in buffer, out EMPTY a, inout EMPTY b,
...
control MyIC(inout EMPTY a, inout EMPTY b, in psa_ingress_input_metadata_t c,
             inout psa_ingress_output_metadata_t d) {
  apply { 
  }
}
...
control MyID(packet_out buffer, out EMPTY a, out EMPTY b, out EMPTY c,
             inout EMPTY d, in EMPTY e, in psa_ingress_output_metadata_t f) {
  apply { 
  }
}
...
PSA_Switch(IngressPipeline(MyIP(), MyIC(), MyID()),
             PacketReplicationEngine(),
             EgressPipeline(MyEP(), MyEC(), MyED()),
             BufferingQueueingEngine())
  main;

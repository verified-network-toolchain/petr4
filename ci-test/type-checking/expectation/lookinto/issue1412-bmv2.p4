/petr4/ci-test/type-checking/testdata/p4_16_samples/issue1412-bmv2.p4
\n
#include <v1model.p4>
...
parser ParserImpl(packet_in buffer, out headers_t parsed_hdr,
                  inout metadata meta, inout standard_metadata_t ostd) {
  state start {
    transition accept;
  }
}
...
control IngressImpl(inout headers_t hdr, inout metadata meta,
                    inout standard_metadata_t ostd) {
  apply { 
  }
}
control EgressImpl(inout headers_t hdr, inout metadata meta,
                   inout standard_metadata_t ostd) {
  action set_true() {
    if (meta.field==0) {
      meta.cond = true;
...
V1Switch(ParserImpl(), VerifyChecksumImpl(), IngressImpl(), EgressImpl(),
           ComputeChecksumImpl(), DeparserImpl())
  main;

/petr4/ci-test/type-checking/testdata/p4_16_samples/issue561-6-bmv2.p4
\n
#include <core.p4>
#include <v1model.p4>
...
control ingress(inout headers hdr, inout metadata meta,
  ...
  apply
    ...
    debug_hdr.apply();
    if (hdr.u[0].short.isValid()) {
      hdr.u[0].short.data = 65535;
    }
    if (hdr.u[0].byte.isValid()) {
      hdr.u[0].byte.data = 170;
    }
    if (hdr.u[1].short.isValid()) {
      hdr.u[1].short.data = 65535;
    }
    if (hdr.u[1].byte.isValid()) {
      hdr.u[1].byte.data = 170;
    }
  }
}

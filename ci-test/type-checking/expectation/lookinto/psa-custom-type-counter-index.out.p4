/petr4/ci-test/type-checking/testdata/p4_16_samples/psa-custom-type-counter-index.p4
\n
#include <core.p4>
#include <bmv2/psa.p4>
...
parser MyIP(packet_in buffer, out headers_t hdr, inout EMPTY b,
            in psa_ingress_parser_input_metadata_t c, in EMPTY_H d,
            in EMPTY_H e) {
  state start {
    buffer.extract(hdr.ethernet);
    transition accept;
...
parser MyEP(packet_in buffer, out EMPTY a, inout EMPTY b,
            in psa_egress_parser_input_metadata_t c, in EMPTY d,
            in EMPTY_H e, in EMPTY_H f) {
  state start {
    transition accept;
  }
}
...
control MyID(packet_out buffer, out EMPTY_H a, out EMPTY_H b, out EMPTY c,
             inout headers_t hdr, in EMPTY e,
             in psa_ingress_output_metadata_t f) {
  apply { 
  }
}
control MyED(packet_out buffer, out EMPTY_H a, out EMPTY_H b, inout EMPTY c,

/petr4/ci-test/type-checking/testdata/p4_16_samples/psa-e2e-cloning-basic-bmv2.p4
\n
/*
...
control cIngress(inout headers_t hdr, inout metadata_t user_meta,
  ...
  apply
    {
    if
      (hdr.ethernet.dstAddr==(EthernetAddress) 8 && istd.packet_path!=PSA_PacketPath_t.RECIRCULATE)
      {
      send_to_port(ostd, PSA_PORT_RECIRCULATE);
    }else
    {
...
control cEgress(inout headers_t hdr, inout metadata_t user_meta,
  ...
  apply
    ...
    if (istd.egress_port==PSA_PORT_RECIRCULATE)
      {
      hdr.ethernet.srcAddr = (EthernetAddress) 48879;
      ostd.clone_session_id = (CloneSessionId_t) (CloneSessionIdUint_t) 10;
    }else
    {
    if (hdr.ethernet.dstAddr==(EthernetAddress) 8)

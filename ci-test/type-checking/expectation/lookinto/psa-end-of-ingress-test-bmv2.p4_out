/petr4/ci-test/type-checking/testdata/p4_16_samples/psa-end-of-ingress-test-bmv2.p4
\n
/*
...
control cIngress(inout headers_t hdr, inout metadata_t user_meta,
  ...
  apply
    ...
    (MulticastGroup_t) (MulticastGroupUint_t) hdr.ethernet.dstAddr[31:16];
    ostd.egress_port = (PortId_t) (PortIdUint_t) hdr.ethernet.dstAddr[15:0];
    packet_path_to_int.apply(istd.packet_path, hdr.output_data.word0);
  }
}
parser EgressParserImpl(packet_in pkt, out headers_t hdr,
                        inout metadata_t user_meta,
                        in psa_egress_parser_input_metadata_t istd,
                        in empty_metadata_t normal_meta,
                        in empty_metadata_t clone_i2e_meta,
...
control cEgress(inout headers_t hdr, inout metadata_t user_meta,
  ...
  apply
    ...
    bit<8> idx = hdr.ethernet.etherType[7:0];
    bit<16> cur_count = egress_pkt_seen.read(idx);
    if (hdr.ethernet.etherType[15:8]==192)
      {
      hdr.output_data.word0 = (bit<32>) cur_count;
    }
       else
       if (hdr.ethernet.etherType[15:8]==193)
         {
         bit<16> write_data = hdr.ethernet.srcAddr[15:0];
         egress_pkt_seen.write(idx, write_data);
       }else
       ...
       hdr.output_data.word1 = (bit<32>) istd.egress_port;
       hdr.output_data.word2 =
       (bit<32>) (EgressInstanceUint_t) istd.instance;
       packet_path_to_int.apply(istd.packet_path, hdr.output_data.word3);
       }
  }
}

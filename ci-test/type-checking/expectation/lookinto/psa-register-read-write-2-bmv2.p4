/petr4/ci-test/type-checking/testdata/p4_16_samples/psa-register-read-write-2-bmv2.p4
\n
#include <core.p4>
#include <bmv2/psa.p4>
...
parser MyIP(packet_in pkt, out headers_t hdr, inout metadata_t user_meta,
            in psa_ingress_parser_input_metadata_t istd,
            in EMPTY resubmit_meta, in EMPTY recirculate_meta) {
  state start
    {
    pkt.extract(hdr.ethernet);
    pkt.extract(hdr.output_data);
...
parser MyEP(packet_in pkt, out headers_t hdr, inout metadata_t user_meta,
            in psa_egress_parser_input_metadata_t istd, in EMPTY normal_meta,
            in EMPTY clone_i2e_meta, in EMPTY clone_e2e_meta) {
  state start {
    transition accept;
  }
}
control MyIC(inout headers_t hdr, inout metadata_t user_meta,
             in psa_ingress_input_metadata_t istd,
             inout psa_ingress_output_metadata_t ostd) {
  Register<bit<16>, bit<8>>(6) reg;
  bit<8> idx;
...
control MyEC(inout headers_t hdr, inout metadata_t user_meta,
             in psa_egress_input_metadata_t istd,
             inout psa_egress_output_metadata_t ostd) {
  apply { 
  }
}
control MyID(packet_out pkt, out EMPTY clone_i2e_meta,
             out EMPTY resubmit_meta, out EMPTY normal_meta,
             inout headers_t hdr, in metadata_t user_meta,
             in psa_ingress_output_metadata_t istd) {
  apply {
    pkt.emit(hdr.ethernet);
    pkt.emit(hdr.output_data);
...
control MyED(packet_out pkt, out EMPTY clone_e2e_meta,
             out EMPTY recirculate_meta, inout headers_t hdr,
             in metadata_t user_meta, in psa_egress_output_metadata_t istd,
             in psa_egress_deparser_input_metadata_t edstd) {
  apply { 
  }
}
...
extern DirectCounter<W> {
                     ^
/petr4/ci-test/type-checking/testdata/p4_16_samples/psa-register-read-write-2-bmv2.p4(85): [--Wwarn=uninitialized_use] warning: orig_data may be uninitialized
                next_data = orig_data;
                            ^^^^^^^^^
/petr4/ci-test/type-checking/testdata/p4_16_samples/psa-register-read-write-2-bmv2.p4(88): [--Wwarn=uninitialized_use] warning: orig_data may be uninitialized
                next_data = orig_data + 1;
                            ^^^^^^^^^
/petr4/ci-test/type-checking/testdata/p4_16_samples/psa-register-read-write-2-bmv2.p4(95): [--Wwarn=uninitialized_use] warning: orig_data may be uninitialized
            hdr.output_data.word0 = (bit<32>) orig_data;
                                              ^^^^^^^^^

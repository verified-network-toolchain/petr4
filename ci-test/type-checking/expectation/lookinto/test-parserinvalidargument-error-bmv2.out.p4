/petr4/ci-test/type-checking/testdata/p4_16_samples/test-parserinvalidargument-error-bmv2.p4
\n
/*
  ...
  apply
    ...
    if (stdmeta.parser_error==NoError) {
      error_as_int = 0;
       ...
       if (stdmeta.parser_error==PacketTooShort) {
         error_as_int = 1;
       }
          else
          if (stdmeta.parser_error==NoMatch) {
            error_as_int = 2;
          }
             else
             if (stdmeta.parser_error==StackOutOfBounds) {
               error_as_int = 3;
             }
                else
                if (stdmeta.parser_error==HeaderTooShort) {
                  error_as_int = 4;
                }
                   else
                   if (stdmeta.parser_error==ParserTimeout)
                     {
                     error_as_int = 5;
                   }
                      else
                      if (stdmeta.parser_error==ParserInvalidArgument)
                        {
                        error_as_int = 6;
                      }else {
                      error_as_int = 7;

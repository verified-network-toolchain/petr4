/petr4/ci-test/type-checking/testdata/p4_16_samples/psa-example-dpdk-externs.p4
\n
#include <core.p4>
#include <dpdk/psa.p4>
...
extern Counter<W8, S> {
  ...
  void count(in S index, @optional
                         in bit<32> increment);
}
...
extern Meter<S10> {
  ...
  PSA_MeterColor_t execute(in S10 index,
                           ...
                           @optional
                           in bit<32> pkt_len);
  PSA_MeterColor_t execute(in S10 index, @optional
                                         in bit<32> pkt_len);
}
...
control ingress(inout headers hdr, inout metadata_t user_meta,
                in psa_ingress_input_metadata_t istd,
                inout psa_ingress_output_metadata_t ostd) {
  Counter<bit<10>, bit<12>>(1024, PSA_CounterType_t.PACKETS_AND_BYTES)
    counter0;
  Counter<bit<10>, bit<12>>(1024, PSA_CounterType_t.PACKETS) counter1;
  Counter<bit<10>, bit<12>>(1024, PSA_CounterType_t.BYTES) counter2;
  Register<bit<32>, bit<12>>(1024) reg;
...
control egress(inout headers hdr, inout metadata_t user_meta,
               in psa_egress_input_metadata_t istd,
               inout psa_egress_output_metadata_t ostd) {
  apply { 
  }
...
control IngressDeparserImpl(packet_out packet,
                            ...
                            inout headers hdr, in metadata_t meta,
                            in psa_ingress_output_metadata_t istd) {
  apply {
    packet.emit(hdr.ethernet);
    packet.emit(hdr.ipv4);
...
/petr4/ci-test/type-checking/p4include/dpdk/psa.p4(534): [--Wwarn=unused] warning: 'W' is unused
extern Counter<W, S> {
               ^
/petr4/ci-test/type-checking/p4include/dpdk/psa.p4(563): [--Wwarn=unused] warning: 'W' is unused

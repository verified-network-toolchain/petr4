/petr4/ci-test/type-checking/testdata/p4_16_samples/psa-example-logical-operations.p4
\n
#include <core.p4>
#include <bmv2/psa.p4>
...
parser MyIP(packet_in buffer, out ethernet_t h, inout metadata b,
...
parser MyEP(packet_in buffer, out EMPTY_H a, inout metadata b,
...
control MyIC(inout ethernet_t a, inout metadata b,
             in psa_ingress_input_metadata_t c,
             inout psa_ingress_output_metadata_t d) {
  action forward()
    {
...
control MyEC(inout EMPTY_H a, inout metadata b,
             in psa_egress_input_metadata_t c,
             inout psa_egress_output_metadata_t d) {
  apply { 
  }
...
control MyID(packet_out buffer, out EMPTY_CLONE a, out EMPTY_RESUB b,
             out EMPTY_BRIDGE c, inout ethernet_t d, in metadata e,
             in psa_ingress_output_metadata_t f) {
  apply { 
  }
}
control MyED(packet_out buffer, out EMPTY_CLONE a, out EMPTY_RECIRC b,
             inout EMPTY_H c, in metadata d,
             in psa_egress_output_metadata_t e,
             in psa_egress_deparser_input_metadata_t f) {
  apply { 
  }

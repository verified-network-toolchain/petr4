/petr4/ci-test/type-checking/testdata/p4_16_samples/psa-table-hit-miss.p4
\n
#include <core.p4>
#include <bmv2/psa.p4>
...
parser MyIP(packet_in buffer, out headers_t hdr, inout EMPTY_M b,
            in psa_ingress_parser_input_metadata_t c, in EMPTY_RESUB d,
            in EMPTY_RECIRC e) {
  state start {
    buffer.extract(hdr.ethernet);
    transition accept;
...
parser MyEP(packet_in buffer, out EMPTY_H a, inout EMPTY_M b,
            in psa_egress_parser_input_metadata_t c, in EMPTY_BRIDGE d,
            in EMPTY_CLONE e, in EMPTY_CLONE f) {
  state start {
    transition accept;
  }
}
control MyIC(inout headers_t hdr, inout EMPTY_M b,
             in psa_ingress_input_metadata_t c,
             inout psa_ingress_output_metadata_t d) {
  action remove_header() {
    hdr.ethernet.setInvalid();
...
control MyEC(inout EMPTY_H a, inout EMPTY_M b,
             in psa_egress_input_metadata_t c,
             inout psa_egress_output_metadata_t d) {
  apply { 
  }
}
control MyID(packet_out buffer, out EMPTY_CLONE a, out EMPTY_RESUB b,
             out EMPTY_BRIDGE c, inout headers_t hdr, in EMPTY_M e,
             in psa_ingress_output_metadata_t f) {
  apply { 
  }
}
control MyED(packet_out buffer, out EMPTY_CLONE a, out EMPTY_RECIRC b,
             inout EMPTY_H c, in EMPTY_M d,
             in psa_egress_output_metadata_t e,
             in psa_egress_deparser_input_metadata_t f) {
  apply { 
  }
...
extern DirectCounter<W> {
                     ^
/petr4/ci-test/type-checking/testdata/p4_16_samples/psa-table-hit-miss.p4(62): [--Wwarn=invalid_header] warning: accessing a field of a potentially invalid header hdr.ethernet
            hdr.ethernet.srcAddr : exact;
            ^^^^^^^^^^^^

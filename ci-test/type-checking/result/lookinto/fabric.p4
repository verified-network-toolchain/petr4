/petr4/ci-test/type-checking/testdata/p4_16_samples/fabric_20190420/fabric.p4
\n
/*
...
const bit<16> ETHERTYPE_QINQ = 34984;
const bit<16> ETHERTYPE_QINQ_NON_STD = 37120;
const bit<16> ETHERTYPE_VLAN = 33024;
const bit<16> ETHERTYPE_MPLS = 34887;
const bit<16> ETHERTYPE_MPLS_MULTICAST = 34888;
const bit<16> ETHERTYPE_IPV4 = 2048;
const bit<16> ETHERTYPE_IPV6 = 34525;
const bit<16> ETHERTYPE_ARP = 2054;
...
control Forwarding(inout parsed_headers_t hdr,
  ...
  table bridging
    ...
    actions = {
      set_next_id_bridging;@defaultonly
      nop;
    }
  ...
  table routing_v4
    ...
    actions = {
      set_next_id_routing_v4;nop_routing_v4;@defaultonly
      nop;
    }
    const default_action = nop();
...
control Acl(inout parsed_headers_t hdr,
  ...
  table acl
    ...
    actions = {
      set_next_id_acl;punt_to_cpu;clone_to_cpu;drop;nop_acl;
    }
...
control Next(inout parsed_headers_t hdr,
  ...
  @hidden
  action
    mpls_routing(port_num_t port_num,
                 mac_addr_t smac,
                 mac_addr_t dmac,
                 mpls_label_t label)
    {
  ...
  table xconnect
    ...
    actions = {
      output_xconnect;set_next_id_xconnect;@defaultonly
      nop;
    }
  ...
  action
    routing_hashed(port_num_t port_num, mac_addr_t smac, mac_addr_t dmac)
    {
    routing(port_num, smac, dmac);
    hashed_counter.count();
  }
  action
    mpls_routing_hashed(port_num_t port_num,
                        mac_addr_t smac,
                        mac_addr_t dmac,
                        mpls_label_t label)
    {
  ...
  table hashed
    ...
    actions =
      {
      output_hashed;routing_hashed;mpls_routing_hashed;@defaultonly
      nop;
    }
  ...
  table multicast
    ...
    actions = {
      set_mcast_group_id;@defaultonly
      nop;
    }
...
control EgressNextControl(inout parsed_headers_t hdr,
  ...
  apply
    {
    if
      (fabric_metadata.is_multicast==true && standard_metadata.ingress_port==standard_metadata.egress_port)
      {
      mark_to_drop(standard_metadata);
    }
...
control PacketIoEgress(inout parsed_headers_t hdr,
  ...
  apply
    ...
    if (standard_metadata.egress_port==255)
      {
      if
        (fabric_metadata.is_multicast==true && fabric_metadata.clone_to_cpu==false)
        {
        mark_to_drop(standard_metadata);
      }
...
control spgw_normalizer(in bool is_gtpu_encapped, out ipv4_t gtpu_ipv4,
                        out udp_t gtpu_udp, inout ipv4_t ipv4,
                        inout udp_t udp, in ipv4_t inner_ipv4,
                        in udp_t inner_udp) {
  apply
    {
    if (!is_gtpu_encapped) 
...
control spgw_ingress(inout ipv4_t gtpu_ipv4, inout udp_t gtpu_udp,
                     inout gtpu_t gtpu, inout ipv4_t ipv4, inout udp_t udp,
  ...
  action
    set_dl_sess_info(bit<32> teid, bit<32> s1u_enb_addr, bit<32> s1u_sgw_addr)
    {
...
control spgw_egress(in ipv4_t ipv4, inout ipv4_t gtpu_ipv4,
                    inout udp_t gtpu_udp, inout gtpu_t gtpu,
...
control update_gtpu_checksum(inout ipv4_t gtpu_ipv4, inout udp_t gtpu_udp,
                             in gtpu_t gtpu, in ipv4_t ipv4, in udp_t udp) {
  apply
    {
    update_checksum(gtpu_ipv4.isValid(),
                      {gtpu_ipv4.version, gtpu_ipv4.ihl, gtpu_ipv4.dscp,
                        gtpu_ipv4.ecn, gtpu_ipv4.total_len,
                        gtpu_ipv4.identification, gtpu_ipv4.flags,
                        gtpu_ipv4.frag_offset, gtpu_ipv4.ttl,
                        gtpu_ipv4.protocol, gtpu_ipv4.src_addr,
                        gtpu_ipv4.dst_addr},
                      gtpu_ipv4.hdr_checksum, HashAlgorithm.csum16);
  }
}
  ...
  apply
    ...
    update_checksum(hdr.ipv4.isValid(),
                      {hdr.ipv4.version, hdr.ipv4.ihl, hdr.ipv4.dscp,
                        hdr.ipv4.ecn, hdr.ipv4.total_len,
...
parser FabricParser(packet_in packet, out parsed_headers_t hdr,
                    inout fabric_metadata_t fabric_metadata,
                    inout standard_metadata_t standard_metadata) {
...
control FabricIngress(inout parsed_headers_t hdr,
  ...
  apply
    ...
    pkt_io_ingress.apply(hdr, fabric_metadata, standard_metadata);
    filtering.apply(hdr, fabric_metadata, standard_metadata);
    spgw_ingress.apply(hdr.gtpu_ipv4, hdr.gtpu_udp, hdr.gtpu, hdr.ipv4,
                         hdr.udp, fabric_metadata, standard_metadata);
...
V1Switch(FabricParser(), FabricVerifyChecksum(), FabricIngress(),
           FabricEgress(), FabricComputeChecksum(), FabricDeparser())
  main;

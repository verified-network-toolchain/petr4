/petr4/ci-test/type-checking/testdata/p4_16_samples/issue486-bmv2.p4
\n
/*
...
parser parserI(packet_in pkt, out Parsed_packet hdr, inout metadata m,
               inout standard_metadata_t stdmeta) {
  state start {
    pkt.extract(hdr.x);
    transition accept;
...
control cIngress(inout Parsed_packet hdr, inout metadata m,
                 inout standard_metadata_t stdmeta) {
  bit<32> z = 5;
  action foo() { 
  }
...
control cEgress(inout Parsed_packet hdr, inout metadata m,
                inout standard_metadata_t stdmeta) {
  apply { 
  }
}
...
V1Switch<Parsed_packet, metadata>(parserI(), vc(), cIngress(), cEgress(),
                                    uc(), DeparserI())
  main;

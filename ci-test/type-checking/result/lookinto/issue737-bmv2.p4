/petr4/ci-test/type-checking/testdata/p4_16_samples/issue737-bmv2.p4
\n
#include <v1model.p4>
...
control IngressP(inout Parsed_packet hdr, inout Meta m,
                 inout standard_metadata_t standard_metadata) {
  apply
    {
    if (m.metafield) {
...
control EgressP(inout Parsed_packet hdr, inout Meta meta,
                inout standard_metadata_t standard_metadata) {
  apply { 
  }
}
...
V1Switch(TopParser(), VeryChecksum(), IngressP(), EgressP(),
           ChecksumComputer(), TopDeparser())
  main;

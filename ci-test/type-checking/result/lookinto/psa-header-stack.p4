/petr4/ci-test/type-checking/testdata/p4_16_samples/psa-header-stack.p4
\n
#include <core.p4>
#include <bmv2/psa.p4>
...
parser MyIP(packet_in buffer, out header_t h, inout EMPTY_M b,
...
control MyIC(inout header_t a, inout EMPTY_M b,
             in psa_ingress_input_metadata_t c,
             inout psa_ingress_output_metadata_t d) {
  action forward() {
    d.egress_port = (PortId_t) 1;
...
control MyID(packet_out buffer, out EMPTY_CLONE a, out EMPTY_RESUB b,
             out EMPTY_BRIDGE c, inout header_t d, in EMPTY_M e,
             in psa_ingress_output_metadata_t f) {
  apply
    {
    buffer.emit(d.ethernet);

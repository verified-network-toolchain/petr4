/petr4/ci-test/type-checking/testdata/p4_16_samples/pvs-struct-3-bmv2.p4
\n
#include <v1model.p4>
...
parser MyParser(packet_in b, out my_packet p, inout my_metadata m,
                inout standard_metadata_t s) {
  value_set<value_set_t>(4) pvs;
  state start
    {
    b.extract(p.data);
    transition select(p.data.da[15:0], p.data.db[7:5], p.data.db[29:24],
      p.data.da[30:26]) {
      pvs: accept;
      (2064, 4 &&& 6, 50 &&& 51, 10 .. 20): foo;
    }
  }
  state foo {
    transition accept;
...
control MyIngress(inout my_packet p, inout my_metadata meta,
                  inout standard_metadata_t s) {
  action set_data() { 
  }
  table t {
...
control MyEgress(inout my_packet p, inout my_metadata m,
                 inout standard_metadata_t s) {
  apply { 
  }
}

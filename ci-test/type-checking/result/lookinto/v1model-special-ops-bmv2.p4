/petr4/ci-test/type-checking/testdata/p4_16_samples/v1model-special-ops-bmv2.p4
\n
/*
...
control fill_ipv4_address(out bit<32> ipv4_address, in bit<8> byte0,
                          in bit<8> byte1, in bit<8> byte2, in bit<8> byte3) {
  apply {
    ipv4_address = byte0 ++ byte1 ++ byte2 ++ byte3;
  }
}
control ingress(inout headers_t hdr, inout meta_t meta,
                inout standard_metadata_t standard_metadata) {
  fill_ipv4_address() c_fill_ipv4_address;
  const bit<32> RESUBMITTED_PKT_L2PTR = 58635;
  const bit<32> RECIRCULATED_PKT_L2PTR = 60444;
  action set_l2ptr(bit<32> l2ptr) {
    meta.fwd.l2ptr = l2ptr;
  }
  action set_mcast_grp(bit<16> mcast_grp)
  ...
  table ipv4_da_lpm
    ...
    actions =
      {
      set_l2ptr;set_mcast_grp;do_resubmit;do_clone_i2e;my_drop(standard_metadata);
    }
    default_action = my_drop(standard_metadata);
  }
  action set_bd_dmac_intf(bit<24> bd, bit<48> dmac, bit<9> intf)
...
control egress(inout headers_t hdr, inout meta_t meta,
  ...
  table send_frame
    ...
    actions =
      {
      rewrite_mac;do_recirculate;do_clone_e2e;my_drop(standard_metadata);
    }
    default_action = my_drop(standard_metadata);
  }
  apply
    ...
    if
      ...
      {
      hdr.switch_to_cpu.setValid();
      hdr.switch_to_cpu.word0 = 19792174;
      hdr.switch_to_cpu.word1 = 1515870810;
    }
       else
       if
         (standard_metadata.instance_type==BMV2_V1MODEL_INSTANCE_TYPE_EGRESS_CLONE)
         {
         hdr.switch_to_cpu.setValid();
         hdr.switch_to_cpu.word0 = 237899310;
         hdr.switch_to_cpu.word1 = 1515870810;
       }else
       {
       if
         (standard_metadata.instance_type==BMV2_V1MODEL_INSTANCE_TYPE_REPLICATION)
         {
         get_multicast_copy_out_bd.apply();
       }
       send_frame.apply();

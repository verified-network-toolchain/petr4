...
control pipe(inout Headers_t headers, inout metadata meta, inout standard_metadata std_meta) {
    @name("old_addr") bit<32> old_addr;
    @name("new_addr") bit<32> new_addr;
    @name("from") bit<16> from;
    @name("to") bit<16> to;
    apply {
        old_addr = headers.ipv4.dstAddr;
        new_addr = 32w0x1020304;
        headers.ipv4.dstAddr = new_addr;
        headers.ipv4.checksum = csum_replace4(headers.ipv4.checksum, old_addr, new_addr);
        from = headers.udp.dstPort;
        to = 16w0x400;

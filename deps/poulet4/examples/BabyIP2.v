    ...
    [(MkDeclarationField NoInfo
          (TypHeader
           [( {| stags := NoInfo; str := "src" |}, (TypBit 8) );
            ...
            ( {| stags := NoInfo; str := "t_or_u_sport" |}, (TypBit 8) );
            ( {| stags := NoInfo; str := "t_or_u_dport" |}, (TypBit 8) );
            ( {| stags := NoInfo; str := "t_or_u_flags" |}, (TypBit 4) )])
          {| stags := NoInfo; str := "comb" |});
     (MkDeclarationField NoInfo
          (TypHeader [( {| stags := NoInfo; str := "seq" |}, (TypBit 8) )])

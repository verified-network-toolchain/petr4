...
Section CComp.
  ...
  Fixpoint CTranslateExpr (e: E.e tags_t) (env: ClightEnv)
    ...
    | <{Var x : ty @ i}> => (*first find if x has been declared. If not, declare it by putting it into vars*)
                        ...
                        | Some id => Some ([], Evar id (CTranslateType ty), env)
                        | _ => None
                        end 
    | <{Slice n : τ [ hi : lo ] @ i}> => 
                        match CTranslateExpr n env with
                        | Some (stmts, n', env') => prepend_stmts stmts (CTranslateSlice n' hi lo τ env')
                        | _ => None
                        end
    | <{Cast e : τ @ i}> => 
                        match CTranslateExpr e env with
                        | Some (stmts, e', env') => let typefrom := (CubTypeOf e) in 
                                      prepend_stmts stmts (CTranslateCast e' typefrom τ env')
                        | _ => None
                        end
    | <{UOP op x : ty @ i}> => 
                        ...
                        | Some (stmts, x', env') => 
                          match op with
                          | _{!}_ => Some (stmts, Eunop Onotbool x' (CTranslateType ty), env')
                          | _{~}_ => Some (stmts, Eunop Onotint x' (CTranslateType ty), env')
                          | _{-}_ => Some (stmts, Eunop Oneg x' (CTranslateType ty), env')
    ...
    | <{BOP x : tx op y : ty @ i}> =>
                        ...
                        | Some (stmts_x, x', env') =>  
                          ...
                          | Some (stmts_y, y', env'') => 
                            match op with
                            | +{+}+ =>  Some (stmts_x ++ stmts_y, Ebinop Oadd x' y' (CTranslateType tx), env'')
                            | +{-}+ =>  Some (stmts_x ++ stmts_y, Ebinop Osub x' y' (CTranslateType tx), env'')
                            | +{|+|}+ =>None
                            | +{|-|}+ =>None
                            | E.Times =>  Some (stmts_x ++ stmts_y, Ebinop Omul x' y' (CTranslateType tx), env'')
                            | +{<<}+ => Some (stmts_x ++ stmts_y, Ebinop Oshl x' y' (CTranslateType tx), env'')
                            | +{>>}+ => Some (stmts_x ++ stmts_y, Ebinop Oshr x' y' (CTranslateType tx), env'')
                            | +{<=}+ => Some (stmts_x ++ stmts_y, Ebinop Ole x' y' type_bool, env'')                         
                            | +{>=}+ => Some (stmts_x ++ stmts_y, Ebinop Oge x' y' type_bool, env'')
                            | +{<}+ =>  Some (stmts_x ++ stmts_y, Ebinop Olt x' y' type_bool, env'')
                            | +{>}+ =>  Some (stmts_x ++ stmts_y, Ebinop Ogt x' y' type_bool, env'')
                            | +{==}+ => Some (stmts_x ++ stmts_y, Ebinop Oeq x' y' type_bool, env'')
                            | +{!=}+ => Some (stmts_x ++ stmts_y, Ebinop One x' y' type_bool, env'')
                            | +{&&}+
                            | +{&}+ =>  Some (stmts_x ++ stmts_y, Ebinop Oand x' y' (CTranslateType tx), env'')
                            | +{^}+ =>  Some (stmts_x ++ stmts_y, Ebinop Oxor x' y' (CTranslateType tx), env'')
                            | +{||}+
                            | +{|}+ =>  Some (stmts_x ++ stmts_y, Ebinop Oor x' y' (CTranslateType tx), env'')
                            | +{++}+ => (*x ++ y = x<< widthof(y) + y*)
                                        let shift_amount := Econst_long (Integers.Int64.repr (Z.of_nat (E.width_of_typ ty))) long_unsigned in 
                                        Some (stmts_x ++ stmts_y, Ebinop Oadd (Ebinop Oshl x' shift_amount (CTranslateType tx)) y' (CTranslateType tx), env'')

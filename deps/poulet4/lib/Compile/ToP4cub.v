...
Section ToP4cub.
  ...
  Definition translate_actions_loop
    ...
    (action : TableActionRef) (acc : result (list (string * Expr.args)))
        : result (list (string * Expr.args)) :=
  ...
  Definition translate_actions
    (decls : DeclCtx) (term_names : list string)
    (actions : list TableActionRef) : result (list (string * Expr.args)) :=
    List.fold_right

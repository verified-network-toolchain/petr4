...
Section WeakestPre.
  ...
  Section weakest_precondition_statements.
    Variable (weakest_precondition_statement: @Statement tags_t -> @pred environment -> @pred environment).
  
    Fixpoint weakest_precondition_statements (stmts: list (@Statement tags_t)) (post: @pred environment) :=
      match stmts with
      | nil => post
      | stmt :: stmts' =>
  ...
  Definition weakest_precondition_statements_correct
      (stmts: list (@Statement tags_t))
  :=
      forall env_pre post,
          weakest_precondition_statements (weakest_precondition_statement) stmts post env_pre ->
              match eval_statements tags_t (eval_statement tags_t tags_dummy) stmts env_pre with
              | (inl tt, env_post) => post env_post
              | _ => True
              end
  .
  ...
  Proof.
    ...
    - unfold weakest_precondition_statements_correct; intros.
      ...
      destruct u.
      apply IHstmt0 in H.
      destruct (eval_statements tags_t (eval_statement tags_t tags_dummy) rest e).
      assumption.
    - unfold weakest_precondition_block_correct; intros.
      ...
      destruct (stack_push tags_t env_pre) eqn:?.
      destruct s; try trivial.
      apply IHstmt in H.
      destruct (eval_statements tags_t (eval_statement tags_t tags_dummy) statements e) eqn:?.
      destruct s; try trivial.
      unfold pred_env_popped in H.
      destruct (stack_pop tags_t e0) eqn:?.
      destruct s; try trivial.
      destruct u0, u1.
      assumption.
  Qed.

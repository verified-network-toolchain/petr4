...
Definition matchrow_inner (table : string) (n : nat) (elt : nat * BV.t * E.matchkind) (acc_res : result string F.t) : result string F.t :=
  let (te, mk) := elt in
  let (w, k) := te in
...
Definition matchrow (table : string) (keys : list (nat * BV.t * E.matchkind)) : result string F.t :=
...
Definition action_inner (table : string) (keys : list (nat * BV.t * E.matchkind)) (w : nat) (n : nat) (named_action : string * ToGCL.target) (res_acc : result string (ToGCL.target)) : result string ToGCL.target :=
...
Definition actions_encoding (table : string) (keys : list (nat * BV.t * E.matchkind)) (actions : list (string * ToGCL.target)) : result string ToGCL.target :=
  let w := bits_to_encode_list_index actions in
  fold_lefti (action_inner table keys w) (ok (G.GAssume (F.LBool false))) actions.

Definition instr {tags_t} (table : string) (i : tags_t) (keys: list (nat * BV.t * E.matchkind)) (actions: list (string * ToGCL.target)) : result string ToGCL.target :=

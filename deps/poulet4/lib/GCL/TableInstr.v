...
Definition action_inner (table : string) (keys : list (nat * BV.t * E.matchkind)) (w : nat) (n : nat) (named_action : string * ((list BV.t) * ToGCL.target)) (res_acc : result (ToGCL.target)) : result ToGCL.target :=
...
Definition actions_encoding (table : string) (keys : list (nat * BV.t * E.matchkind)) (actions : list (string * ((list BV.t) * ToGCL.target))) : result ToGCL.target :=
  let w := bits_to_encode_list_index actions in
  fold_lefti (action_inner table keys w) (ok (G.GAssume (F.LBool false))) actions.

Definition instr {tags_t : Type} (table : string) (keys: list (nat * BV.t * E.matchkind)) (actions: list (string * ((list BV.t) * ToGCL.target))) : result ToGCL.target :=

...
Section ToGCL.
  ...
  Definition pipeline : Type := list E.t -> TD.constructor_args  -> result (ST.s ).

  Section Instr.

    Variable instr : (string -> list (nat * BitVec.t * string) -> list (string * target) -> result target).
    ...
    Fixpoint to_lvalue (e : E.e ) : result string :=
      match e with
    ...
    Fixpoint to_header_string (e : E.e) : result string :=
      match e with
    ...
    Fixpoint to_rvalue (e : E.e) : result BV.t :=
      match e with
      | E.Bool b =>
        if b
    ...
    Fixpoint to_form (e : E.e) : result Form.t :=
      match e with
    ...
    Definition cond (guard_type : E.t) (guard : E.e)  (tres fres : (target * ctx)) : result (target * ctx) :=
      let (tg, tctx) := tres in
      let (fg, fctx) := fres in
    ...
    Definition arrowE_to_arglist
      : F.fs string (paramarg E.e E.e)
        -> result (list (string * (Form.t + BV.t))) :=
    ...
    Fixpoint inline_to_gcl (c : ctx) (arch : model) (s : Inline.t ) : result (target * ctx) :=
      match s with
      | Inline.ISkip =>
        ok (GCL.GSkip, c)
    ...
    Definition get_main ctx (pipe : pipeline) : result (ST.s ) :=
    ...
    Definition inlining_passes (gas unroll : nat) (ext : model) (ctx : ToP4cub.DeclCtx ) (s : ST.s ) : result (Inline.t ) :=
    ...
    Definition from_p4cub (gas unroll : nat) (ext : model) (pipe : pipeline) (ctx : ToP4cub.DeclCtx) : result target :=

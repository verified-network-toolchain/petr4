...
Definition ingress_metadata_t := DeclStruct NoInfo
    ...
    [(MkDeclarationField NoInfo (TypBit 32%N)
          {| stags := NoInfo; str := "flow_ipg" |});
     (MkDeclarationField NoInfo (TypBit 13%N)
          {| stags := NoInfo; str := "flowlet_map_index" |});
     (MkDeclarationField NoInfo (TypBit 16%N)
          {| stags := NoInfo; str := "flowlet_id" |});
     (MkDeclarationField NoInfo (TypBit 32%N)
          {| stags := NoInfo; str := "flowlet_lasttime" |});
     (MkDeclarationField NoInfo (TypBit 14%N)
          {| stags := NoInfo; str := "ecmp_offset" |});
     (MkDeclarationField NoInfo (TypBit 32%N)
...
Definition intrinsic_metadata_t := DeclStruct NoInfo
    ...
    [(MkDeclarationField NoInfo (TypBit 48%N)
          {| stags := NoInfo; str := "ingress_global_timestamp" |});
     (MkDeclarationField NoInfo (TypBit 32%N)
          {| stags := NoInfo; str := "lf_field_list" |});
     (MkDeclarationField NoInfo (TypBit 16%N)
          {| stags := NoInfo; str := "mcast_grp" |});
     (MkDeclarationField NoInfo (TypBit 16%N)
          {| stags := NoInfo; str := "egress_rid" |})].
...
Definition metadata := DeclStruct NoInfo
    ...
    [(MkDeclarationField NoInfo
          (TypStruct
           [( {| stags := NoInfo; str := "flow_ipg" |}, (TypBit 32%N) );
            ( {| stags := NoInfo; str := "flowlet_map_index" |},
              (TypBit 13%N) );
            ( {| stags := NoInfo; str := "flowlet_id" |}, (TypBit 16%N) );
            ( {| stags := NoInfo; str := "flowlet_lasttime" |},
              (TypBit 32%N) );
            ( {| stags := NoInfo; str := "ecmp_offset" |}, (TypBit 14%N) );
            ( {| stags := NoInfo; str := "nhop_ipv4" |}, (TypBit 32%N) )])
          {| stags := NoInfo; str := "ingress_metadata" |});
     (MkDeclarationField NoInfo
          (TypStruct
           [( {| stags := NoInfo; str := "ingress_global_timestamp" |},
              (TypBit 48%N) );
            ( {| stags := NoInfo; str := "lf_field_list" |}, (TypBit 32%N) );
            ( {| stags := NoInfo; str := "mcast_grp" |}, (TypBit 16%N) );
            ( {| stags := NoInfo; str := "egress_rid" |}, (TypBit 16%N) )])
          {| stags := NoInfo; str := "intrinsic_metadata" |})].
...
Definition ingress := DeclControl NoInfo
    ...
    [(DeclInstantiation NoInfo
          ...
          [(MkExpression NoInfo
                (ExpInt
                 {| itags := NoInfo; value := 8192;
                    width_signed := (Some ( 32%N, false )) |}) (TypBit 32%N)
                Directionless)] {| stags := NoInfo; str := "flowlet_id" |}
          nil);
     (DeclInstantiation NoInfo
          (TypSpecializedType
               (TypTypeName
          ...
          [(MkExpression NoInfo
                (ExpInt
                 {| itags := NoInfo; value := 8192;
                    width_signed := (Some ( 32%N, false )) |}) (TypBit 32%N)
                Directionless)]
          {| stags := NoInfo; str := "flowlet_lasttime" |} nil);
     (DeclAction NoInfo {| stags := NoInfo; str := "_drop" |} nil nil
          (BlockCons
               (MkStatement NoInfo
                    (StatMethodCall
                         (MkExpression NoInfo
                              ...
                              Directionless) nil nil) StmUnit)
               (BlockEmpty NoInfo)));
     (DeclAction NoInfo {| stags := NoInfo; str := "set_ecmp_select" |} nil
          [(MkParameter false Directionless (TypBit 8%N) None
                {| stags := NoInfo; str := "ecmp_base" |});
           (MkParameter false Directionless (TypBit 8%N) None
                {| stags := NoInfo; str := "ecmp_count" |})]
          (BlockCons
               (MkStatement NoInfo
                    (StatMethodCall
                         (MkExpression NoInfo
                                     ...
                                     (MkExpression NoInfo
                                          (ExpExpressionMember
                                               (MkExpression NoInfo
                                                    ...
                                                    InOut)
                                               {| stags := NoInfo;
                                                  str := "ingress_metadata" |})
                                          (TypStruct
                                           [( {| stags := NoInfo;
                                                 str := "flow_ipg" |},
                                              (TypBit 32%N) );
                                            ( {| stags := NoInfo;
                                                 str := "flowlet_map_index" |},
                                              (TypBit 13%N) );
                                            ( {| stags := NoInfo;
                                                 str := "flowlet_id" |},
                                              (TypBit 16%N) );
                                            ( {| stags := NoInfo;
                                                 str := "flowlet_lasttime" |},
                                              (TypBit 32%N) );
                                            ( {| stags := NoInfo;
                                                 str := "ecmp_offset" |},
                                              (TypBit 14%N) );
                                          ...
                                          (ExpName
                                           (BareName
                                            {| stags := NoInfo;
                                               str := "ecmp_count" |})
                                           NoLocator) (TypBit 8%N) In))
                                (TypBit 20%N) Directionless))]) StmUnit)
               (BlockEmpty NoInfo)));
          ...
          (BlockCons
               ...
               (BlockCons
                    ...
                    (BlockCons
                         ...
                         (BlockEmpty NoInfo)))));
     (DeclAction NoInfo {| stags := NoInfo; str := "lookup_flowlet_map" |}
          nil nil
          (BlockCons
               (MkStatement NoInfo
                    (StatMethodCall
                               ...
                               (MkFunctionType
                                    ...
                                    [(MkParameter false Out
                                     ...
                                     (MkParameter false In
                                          ...
                                          {| stags := NoInfo; str := "max" |})]
                                    FunExtern TypVoid)) Directionless)
                         [(TypBit 13%N); (TypBit 13%N);
                          (TypList
                           [(TypBit 32%N); (TypBit 32%N); (TypBit 8%N);
                            (TypBit 16%N); (TypBit 16%N)]); (TypBit 26%N)]
                         [(Some
                           (MkExpression NoInfo
                                (ExpExpressionMember
                                     (MkExpression NoInfo
                                     ...
                                     (MkExpression NoInfo
                                          (ExpInt
                                           {| itags := NoInfo; value := 13;
                                              width_signed := Some (26%N, false) |})
                                          TypInteger Directionless))
                                (TypBit 26%N) Directionless))]) StmUnit)
               (BlockCons
                    (MkStatement NoInfo
                         (StatMethodCall
                              (MkExpression NoInfo
                    ...
                    (StatAssignment
                         ...
                         (MkExpression NoInfo
                              (ExpName
                               ...
                               NoLocator) (TypBit 48%N) In)) StmUnit)
               (BlockEmpty NoInfo)));
     (DeclAction NoInfo {| stags := NoInfo; str := "update_flowlet_id" |} nil
          nil
          (BlockCons
               (MkStatement NoInfo
                    (StatAssignment
     ...
     (DeclTable NoInfo {| stags := NoInfo; str := "ecmp_group" |}
          ...
          [(MkTableActionRef NoInfo
           ...
           (MkTableActionRef NoInfo
                (MkTablePreActionRef
                     (BareName
                      {| stags := NoInfo; str := "set_ecmp_select" |}) nil)
                (TypAction nil
                     [(MkParameter false Directionless (TypBit 8%N) None
                           {| stags := NoInfo; str := "ecmp_base" |});
                      (MkParameter false Directionless (TypBit 8%N) None
     ...
     (DeclTable NoInfo {| stags := NoInfo; str := "ecmp_nhop" |}
          [(MkTableKey NoInfo
                (MkExpression NoInfo
                     (ExpExpressionMember
                          (MkExpression NoInfo
           ...
           (MkTableActionRef NoInfo
                ...
                (TypAction nil
                     [(MkParameter false Directionless (TypBit 32%N) 
                      ...
                      (MkParameter false Directionless (TypBit 9%N) None
                           {| stags := NoInfo; str := "port" |})]))] 
          None None (Some 16384%N) nil);
     (DeclTable NoInfo {| stags := NoInfo; str := "flowlet" |} nil
          [(MkTableActionRef NoInfo
                (MkTablePreActionRef
                     (BareName
                      {| stags := NoInfo; str := "lookup_flowlet_map" |})
     ...
     (DeclTable NoInfo {| stags := NoInfo; str := "new_flowlet" |} nil
          [(MkTableActionRef NoInfo
                (MkTablePreActionRef
                     (BareName
                      {| stags := NoInfo; str := "update_flowlet_id" |}) nil)
                   ...
                   (MkExpression NoInfo
                        (ExpExpressionMember
                             (MkExpression NoInfo
                                  (ExpName
                                   (BareName
                                    {| stags := NoInfo; str := "flowlet" |})
                                   NoLocator)
                                  (TypTable
                                   {| stags := NoInfo;
                                      str := "apply_result_flowlet" |})
                                  Directionless)
                             {| stags := NoInfo; str := "apply" |})
                        (TypFunction
                         (MkFunctionType nil nil FunTable
...
Definition prog := Program
    [decl'1; packet_in; packet_out; verify'check'toSignal; NoAction; decl'2;
     ...
     ComputeChecksum; Deparser; V1Switch; ingress_metadata_t;
     intrinsic_metadata_t; ethernet_t; ipv4_t; tcp_t; metadata; headers;

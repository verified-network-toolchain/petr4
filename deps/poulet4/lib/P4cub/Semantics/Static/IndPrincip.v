...
Section TypeParserExprInduction.
  (** An arbitrary predicate. *)
  Variable P : nat -> expr_type_env -> Parser.e -> Prop.
  
  Hypothesis HGoto : forall total_states Γ st,
      valid_state total_states st ->
      P total_states Γ (Parser.Goto st).
  
  Hypothesis HSelect : forall total_states Γ e default cases τ,
      Γ ⊢ₑ e ∈ τ ->
      type_prsrexpr total_states Γ default ->
      P total_states Γ default ->
      Forall
        (fun pe =>
           let p := fst pe in
           let e := snd pe in
           type_pat p τ /\ type_prsrexpr total_states Γ e) cases ->
      Field.predfs_data (P total_states Γ) cases ->
      P total_states Γ (Parser.Select e default cases).
  
  (** Custom induction principle.
      Do [induction ?H using custom_type_prsrexpr_ind] *)
  Definition custom_type_prsrexpr_ind :
    forall (total_states : nat) (Γ : expr_type_env) (e : Parser.e),
      type_prsrexpr total_states Γ e -> P total_states Γ e :=
  fix teind sts Γ e Hy :=
    let fix lind
            {cases : Field.fs Parser.pat Parser.e} {τ : Expr.t}
            (HR : Forall
                    (fun pe =>
                       let p := fst pe in
                       let e := snd pe in
                       type_pat p τ /\ type_prsrexpr sts Γ e) cases)
      : Field.predfs_data (P sts Γ) cases :=
      match HR with
      | Forall_nil _ => Forall_nil _
      | Forall_cons _ (conj _ He) Htail
        => Forall_cons _ (teind _ _ _ He) (lind Htail)
      end in
    match Hy with
    | type_goto _ _ _ Hst => HGoto _ _ _ Hst
    | type_select _ _ _ _ _ _ He Hd Hcases
      => HSelect _ _ _ _ _ _ He Hd
                (teind _ _ _ Hd) Hcases (lind Hcases)
    end.
End TypeParserExprInduction.

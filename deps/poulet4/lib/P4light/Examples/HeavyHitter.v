...
Definition metadata := DeclStruct NoInfo
    ...
    [(MkDeclarationField NoInfo
          ...
          {| stags := NoInfo; str := "custom_metadata" |})].
...
Definition ingress := DeclControl NoInfo
    ...
    [(DeclInstantiation NoInfo
          (TypSpecializedType
               (TypTypeName {| stags := NoInfo; str := "counter" |}) nil)
          [(MkExpression NoInfo
                (ExpInt
                ...
                Directionless);
           (MkExpression NoInfo
                (ExpTypeMember {| stags := NoInfo; str := "CounterType" |}
                     {| stags := NoInfo; str := "packets" |})
                (TypEnum {| stags := NoInfo; str := "CounterType" |} 
                     ...
                     [{| stags := NoInfo; str := "packets" |};
                      ...
                      {| stags := NoInfo; str := "packets_and_bytes" |}])
                Directionless)]
          {| stags := NoInfo; str := "ip_src_counter" |} nil);
     (DeclAction NoInfo {| stags := NoInfo; str := "count_action" |} nil
          [(MkParameter false Directionless (TypBit 32%N) None
                {| stags := NoInfo; str := "idx" |})]
          (BlockCons
               (MkStatement NoInfo
                    (StatMethodCall
                         (MkExpression NoInfo
          ...
          (BlockCons
               ...
               (BlockCons
                    ...
                    (BlockCons
                         ...
                         (BlockEmpty NoInfo)))));
     (DeclTable NoInfo {| stags := NoInfo; str := "count_table" |}
          [(MkTableKey NoInfo
                (MkExpression NoInfo
                     (ExpExpressionMember
                          (MkExpression NoInfo
          ...
          [(MkTableActionRef NoInfo
                (MkTablePreActionRef
                     (BareName {| stags := NoInfo; str := "count_action" |})
                     nil)
                (TypAction nil
                     [(MkParameter false Directionless (TypBit 32%N) 
                           None {| stags := NoInfo; str := "idx" |})]));
           (MkTableActionRef NoInfo
                (MkTablePreActionRef
                     (BareName {| stags := NoInfo; str := "_drop" |}) nil)
                        ...
                        (ExpExpressionMember
                             (MkExpression NoInfo
                                  (ExpName
                                   (BareName
                                    {| stags := NoInfo;
                                       str := "count_table" |}) NoLocator)
                                  (TypTable
                                   {| stags := NoInfo;
                                      str := "apply_result_count_table" |})
                                  Directionless)
                             {| stags := NoInfo; str := "apply" |})
                        (TypFunction
                         (MkFunctionType nil nil FunTable
...
Definition prog := Program
    [decl'1; packet_in; packet_out; verify'check'toSignal; NoAction; decl'2;
     ...
     ComputeChecksum; Deparser; V1Switch; custom_metadata_t; ethernet_t;
     ipv4_t; tcp_t; metadata; headers; ParserImpl; egress; ingress;
     DeparserImpl; verifyChecksum; computeChecksum; main].

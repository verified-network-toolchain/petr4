...
(* Need a list to allow for overloading, but for most things it will be a singleton. *)
Definition genv_func := PathMap.t (list fundef).
...
Inductive exec_expr (read_one_bit : option bool -> bool -> Prop)
  ...
  | exec_expr_name_var : forall name loc sv this st tag typ dir,
                         ...
                         loc_to_sval loc st = Result.Ok sv ->
                         exec_expr read_one_bit this st
...
Definition find_fundef (p: path) (arity: Z) : option fundef :=
  let* candidates := PathMap.get p (ge_func ge) in
  List.hd_error candidates.

Definition add_fundef (p: path) (f: fundef) (ge_func: genv_func) :=
  match PathMap.get p ge_func with
  | None => PathMap.set p [f] ge_func
  | Some l => PathMap.set p (f :: l) ge_func
  end.
...
Definition lookup_func (this_path : path) (func : @Expression tags_t) (arity: Z) : option (option path * fundef) :=
  ...
  | MkExpression _ (ExpName _ loc) _ _ =>
      match loc with
      | LGlobal p =>
        let* fd := find_fundef p arity in
        Some (Some nil, fd)
      | LInstance p =>
          match PathMap.get this_path ge_inst with
          | Some {| iclass:=class_name; |} =>
            let* fd := find_fundef (class_name :: p) arity in
            Some (None, fd)
          | _ => None
          end
      end
  ...
  | MkExpression _ (ExpExpressionMember expr name) _ _ =>
      ...
      | MkExpression _ (ExpName _ loc) (TypTable _) _ =>
          ...
          | LInstance p =>
              match PathMap.get this_path ge_inst with
              | Some {| iclass:=class_name; |} =>
                  let* fd := find_fundef (class_name :: p ++ [str name]) arity in
                  Some (None, fd)
              | _ => None
              end
          | _ => None (* impossible *)
      ...
      | MkExpression _ (ExpName _ loc) _ _ =>
          ...
          | LGlobal p =>
              ...
              | Some {|iclass:=class_name; ipath:=inst_path|} =>
                  let* fd := find_fundef [class_name; str name] arity in
                  Some (Some inst_path, fd)
              | None => None
              end
          | LInstance p =>
...
with exec_func (read_one_bit : option bool -> bool -> Prop) :
  ...
  | exec_func_table_match : forall obj_path name keys actions actionref retv action default_action const_entries s s',
      ...
      exec_func read_one_bit obj_path s (FTable name keys actions default_action const_entries)
...
Variable instantiate_expr' : forall (ce : cenv) (e : ienv) (expr : @Expression tags_t)
    ...
    Result.result Exn.t (ienv_val * inst_mem * extern_state).
...
Definition instantiate'' (ce : cenv) (e : ienv) (typ : @P4Type tags_t)
  ...
  let* '(_, decl) :=
    Result.from_opt (IdentMap.get class_name ce)
                    (Exn.Other ("couldn't find class " ++ class_name)) in
  ...
  let instantiate_arg
        (acc : Result.result Exn.t _)
        (arg : _)
    : Result.result Exn.t _ :=
    let* '(args, m, s, params) := acc in
    let+ (arg, m, s) :=
      instantiate_expr' ce e arg (match params with
                                  | hd :: _ => p ++ [hd]
                                  | _ => []
                                  end) m s in
...
Fixpoint instantiate_expr'
  ...
  | MkExpression _ (ExpName (BareName name) _) _ _ =>
      ...
      let+ inst :=
        Result.from_opt (IdentMap.get (str name) e)
                        (Exn.NameNotFoundInIEnv (str name))
      in
  ...
  | _ =>
      let+ v :=
       Result.from_opt
          (eval_expr_gen (eval_expr_ienv_hook e) expr)
          (Exn.Other "couldn't eval expression")
      in
      (inr v, m, s)
  end.
...
Fixpoint instantiate_decl' (is_init_block : bool) (ce : cenv) (e : ienv) (decl : @Declaration tags_t)
      (p : path) (m : inst_mem) (s : extern_state) : Result.result Exn.t (ienv * inst_mem * extern_state) :=
  match decl with
...
Definition instantiate_decls'
           (ce : cenv)
           (e : ienv)
           (decls : list (@Declaration tags_t))
           (p : path)
           (m : inst_mem)
           (s : extern_state)
  : Result.result Exn.t (inst_mem * extern_state) :=
  let instantiate_decl''
        (ems : Result.result Exn.t (ienv * inst_mem * extern_state))
        (decl : @Declaration tags_t)
    : Result.result Exn.t (ienv * inst_mem * extern_state) :=
    let* '(e, m, s) := ems in
    instantiate_decl' false ce e decl p m s
  in
  let* '(_, m, s) :=
    fold_left instantiate_decl'' decls (Result.Ok (e, m, s)) in
  mret (m, s).
...
Fixpoint instantiate_global_decls' (decls : list (@Declaration tags_t)) (ce : cenv)
      (e : ienv) (m : inst_mem) (s : extern_state) {struct decls} : Result.result Exn.t (inst_mem * extern_state) :=
  match decls with
  | [] => mret (m, s)
  | decl :: decls' =>
...
Definition instantiate_global_decls (decls : list (@Declaration tags_t)) :
      inst_mem -> extern_state -> Result.result Exn.t (inst_mem * extern_state) :=
  instantiate_global_decls' decls (mk_cenv IdentMap.empty) IdentMap.empty.

Definition instantiate_prog (prog : @program tags_t) : Result.result Exn.t (inst_mem * extern_state) :=
  match prog with
  | Program decls =>
...
Definition load_parser_state (p : path) (ge : genv_func) (state : @ParserState tags_t) : genv_func :=
  ...
  | MkParserState _ name body trans =>
      ...
      add_fundef (p ++ [str name]) (FInternal nil body) ge
  end.
...
Definition no_op_action_ref : @TableActionRef tags_t :=
  let NoAction := {| P4String.str := "NoAction";
                     P4String.tags := dummy_tags |} in
...
Fixpoint load_decl (p : path) (ge : genv_func) (decl : @Declaration tags_t) : genv_func :=
  ...
  | DeclParser _ name type_params params constructor_params locals states =>
      ...
      let ge := add_fundef (p ++ [str name; "accept"]) accept_state ge in
      let ge := add_fundef (p ++ [str name; "reject"]) reject_state ge in
      ...
      add_fundef (p ++ [str name; "apply"]) (FInternal (map (map_fst get_loc_path) params) (block_app init (BlockSingleton stmt))) ge
  | DeclControl _ name type_params params _ locals apply =>
      ...
      let ge := fold_left (load_decl (p ++ [str name])) locals ge in
      let init := block_app init (process_locals locals) in
      add_fundef (p ++ [str name; "apply"]) (FInternal (map (map_fst get_loc_path) params) (block_app init apply)) ge
  | DeclFunction _ _ name type_params params body =>
      let out_params := filter_pure_out (map (fun p => (get_param_name_typ p, get_param_dir p)) params) in
      let init := uninit_out_params [str name] out_params in
      let params := map get_param_name_dir params in
      let params := map (map_fst (fun param => LInstance [str name; param])) params in
      add_fundef (p ++ [str name]) (FInternal (map (map_fst get_loc_path) params) (block_app init body)) ge
  | DeclExternFunction _ _ name _ p4params =>
      let params := map get_param_name_dir p4params in
      add_fundef (p ++ [str name]) (FExternal "" (str name) params) ge
  | DeclExternObject _ name _ methods =>
      let add_method_prototype ge' method :=
        match method with
        | ProtoMethod _ _ mname _ p4params =>
            let params := map get_param_name_dir p4params in
            add_fundef (p ++ [str name; str mname]) (FExternal (str name) (str mname) params) ge'
        | _ => ge
        end
  ...
  | DeclAction _ name params ctrl_params body =>
      ...
      add_fundef (p ++ [str name]) (FInternal (map (map_fst get_loc_path) combined_params) (block_app init body)) ge
  | DeclTable _ name keys actions entries default_action_opt _ _ =>
      ...
      add_fundef (p ++ [str name; "apply"]) table ge
  | _ => ge
  end.
...
Definition get_decl_typ_name (decl: @Declaration tags_t): Result.result Exn.t ident :=
  match decl with
  | DeclHeader _ name _
  ...
  | DeclNewType _ name _ => mret (str name)
  | _ => Result.error (Exn.Other "Called get_decl_type_name on a declaration that does not bind a type name")
  end.

(* TODO: Do we need to consider duplicated type names? *)
Fixpoint add_to_genv_typ (ge_typ: genv_typ)
         (decl: @Declaration tags_t): Result.result Exn.t genv_typ :=
  match decl with
  | DeclHeader tags name fields =>
    mret (IdentMap.set (str name) (TypHeader (conv_decl_fields ge_typ fields)) ge_typ)
  | DeclHeaderUnion tags name fields =>
    mret (IdentMap.set (str name) (TypHeaderUnion (conv_decl_fields ge_typ fields)) ge_typ)
  | DeclStruct tags name fields =>
    mret (IdentMap.set (str name) (TypStruct (conv_decl_fields ge_typ fields)) ge_typ)
  | DeclControlType tags name type_params params =>
    mret (IdentMap.set (str name) (TypControl (MkControlType type_params params)) ge_typ)
  | DeclParserType tags name type_params params =>
    mret (IdentMap.set (str name) (TypParser (MkControlType type_params params)) ge_typ)
  (* TODO: DeclPackageType and TypPackage are inconsistency *)
  | DeclPackageType tags name type_params params =>
    mret (IdentMap.set (str name) (TypPackage type_params nil params) ge_typ)
  | DeclEnum tags name members =>
    mret (IdentMap.set (str name) (TypEnum name None members) ge_typ)
  | DeclSerializableEnum tags carrier name members =>
    mret (IdentMap.set (str name) (TypEnum name (Some carrier) (AList.keys members)) ge_typ)
  ...
  | DeclNewType tags name (inl typ) =>
    ...
    | TypTypeName name2 => match IdentMap.get (str name2) ge_typ with
                           | Some typ2 => mret (IdentMap.set (str name) typ2 ge_typ)
                           | None => Result.error (Exn.TypeNameNotFound (str name2))
                           end
    | _ => mret (IdentMap.set (str name) typ ge_typ)
    end
  | DeclTypeDef tags name (inr decl2)
  | DeclNewType tags name (inr decl2) =>
    let* ge_typ2 := add_to_genv_typ ge_typ decl2 in
    let* name2 := get_decl_typ_name decl2 in
    match IdentMap.get name2 ge_typ2 with
    | Some typ2 => mret (IdentMap.set (str name) typ2 ge_typ2)
    | None => Result.error (Exn.TypeNameNotFound name2)
    end
  | _ => mret ge_typ
  end.

Fixpoint add_decls_to_ge_typ
         (ge_typ: genv_typ)
         (l: list (@Declaration tags_t))
  : Result.result Exn.t genv_typ :=
  match l with
  | [] => mret ge_typ
  | decl :: rest =>
      let* ge_typ' := add_to_genv_typ ge_typ decl in
...
Definition eval_ser_enum_members members :=
  asequence (List.map eval_ser_enum_member members).

Definition add_decl_to_ge_senum
         (ge_senum: genv_senum)
         (decl: @Declaration tags_t)
  : Result.result Exn.t genv_senum :=
  match decl with
  | DeclSerializableEnum tags carrier name members =>
      let* members := eval_ser_enum_members members in
      mret (IdentMap.set (str name) members ge_senum)
  | _ =>
      mret ge_senum
  end.

Fixpoint add_decls_to_ge_senum
         (ge_senum: genv_senum)
         (l: list (@Declaration tags_t))
  : Result.result Exn.t genv_senum :=
  match l with
  | [] => mret ge_senum
  | decl :: rest =>
      let* ge_senum := add_decl_to_ge_senum ge_senum decl in
      add_decls_to_ge_senum ge_senum rest
  end.

Definition gen_ge_typ (prog : @program tags_t) : Result.result Exn.t genv_typ :=
  match prog with
  | Program l =>
...
Definition gen_ge_senum (prog : @program tags_t) : Result.result Exn.t genv_senum :=
  match prog with
  | Program l =>
    add_decls_to_ge_senum IdentMap.empty l
  end.
...
Definition exec_module (ge: genv) (read_one_bit : option bool -> bool -> Prop) (this: path) (st: extern_state) (args: list Val) (st': extern_state) (rets: list Val) (sig: signal) : Prop :=
  ...
  | Some func_inst =>
    match find_fundef ge [func_inst.(iclass); "apply"] (Zlength args) with
    | Some func =>
      exists mem': mem,
...
Definition gen_ge (prog : @program tags_t) : Result.result Exn.t genv :=
...
Definition gen_init_es (prog : @program tags_t) : Result.result Exn.t extern_state :=
  let* ge := gen_am_ge prog in
  gen_init_es' ge prog.

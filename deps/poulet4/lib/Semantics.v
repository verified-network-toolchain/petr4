...
| exec_args_cons_continue : forall p s exp exps dir dirs arg args sig,
                            exec_arg p s exp dir arg SContinue ->
...
with exec_call : path -> inst_mem -> state -> (@Expression tags_t) -> state -> signal -> Prop :=
  ...
  *)
  | exec_call_intro : forall this_path inst_m s tags func args typ dir argvals obj_path fd outvals s' s'' sig sig',
  ...
  (* The only example of non-continue signals during exec_args (after SimplExpr) is hd.extract(hdrs.next).
     So do we need to consider signal during exec_copy_out?
     Specifically, non-continue signals in arguments can only be SReject after SimplExpr, 
     and such signals in arguments can only be only triggered by "next". (Other SReject are triggered
     during extern function calls.) For the above example, the parameter's direction is Out. 
     So maybe it is okay to just handle the signal in exec_args, not exec_copy_out. *)

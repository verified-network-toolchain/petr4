...
Section Unpack.
  ...
  Definition unpack_builtin_func (wrapped: env_monad tags_t (Value tags_t)) : env_monad tags_t (string * ValueLvalue) :=
    let* unwrapped := wrapped in
    match unwrapped with
  ...
  Definition unpack_header (wrapped: env_monad tags_t (Value tags_t)) : env_monad tags_t (list (string * (ValueBase tags_t)) * bool) :=
    let* unwrapped := wrapped in
    match unwrapped with

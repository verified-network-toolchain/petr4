...
Section Unpack.
  ...
  Definition unpack_func (wrapped: env_monad Value) : env_monad (list P4Parameter * ValueFunctionImplementation tags_t) :=
    let* unwrapped := wrapped in
    match unwrapped with
  ...
  Definition unpack_lvalue (wrapped: env_monad Value) : env_monad (ValueLvalue tags_t) :=
    let* unwrapped := wrapped in
    match unwrapped with

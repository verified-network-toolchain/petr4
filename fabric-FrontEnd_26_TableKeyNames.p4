...
const ipv4_addr_t S1U_SGW_PREFIX = (bit<32>)32w2348810240;
...
const fwd_type_t FWD_BRIDGING = (bit<3>)3w0;
const fwd_type_t FWD_MPLS = (bit<3>)3w1;
const fwd_type_t FWD_IPV4_UNICAST = (bit<3>)3w2;
const fwd_type_t FWD_IPV4_MULTICAST = (bit<3>)3w3;
const fwd_type_t FWD_IPV6_UNICAST = (bit<3>)3w4;
const fwd_type_t FWD_IPV6_MULTICAST = (bit<3>)3w5;
const fwd_type_t FWD_UNKNOWN = (bit<3>)3w7;
const vlan_id_t DEFAULT_VLAN_ID = 12w4094;
const bit<8> DEFAULT_MPLS_TTL = 8w64;
const bit<8> DEFAULT_IPV4_TTL = 8w64;
const sdf_rule_id_t DEFAULT_SDF_RULE_ID = (bit<32>)32w0;
const pcc_rule_id_t DEFAULT_PCC_RULE_ID = (bit<32>)32w0;
...
control Forwarding(inout parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    ...
    action pop_mpls_and_next(next_id_t next_id) {
        fabric_metadata.mpls_label = (bit<20>)20w0;
        set_next_id(next_id);
        mpls_counter.count();
    }
    table mpls {
...
control Acl(inout parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    ...
    action punt_to_cpu() {
        standard_metadata.egress_spec = (bit<9>)9w255;
        fabric_metadata.skip_next = true;
        acl_counter.count();
    }
    action clone_to_cpu() {
...
control spgw_egress(in ipv4_t ipv4, inout ipv4_t gtpu_ipv4, inout udp_t gtpu_udp, inout gtpu_t gtpu, in fabric_metadata_t fabric_meta, in standard_metadata_t std_meta) {
    @hidden action gtpu_encap() {
        gtpu_ipv4.setValid();
        gtpu_ipv4.version = (bit<4>)4w4;
        gtpu_ipv4.ihl = 4w5;
        gtpu_ipv4.dscp = (bit<6>)6w0;
        gtpu_ipv4.ecn = (bit<2>)2w0;
        gtpu_ipv4.total_len = ipv4.total_len + 16w36;
        gtpu_ipv4.identification = (bit<16>)16w0x1513;
        gtpu_ipv4.flags = (bit<3>)3w0;
        gtpu_ipv4.frag_offset = (bit<13>)13w0;
        gtpu_ipv4.ttl = 8w64;
        gtpu_ipv4.protocol = 8w17;
        gtpu_ipv4.dst_addr = fabric_meta.spgw.s1u_enb_addr;
        gtpu_ipv4.src_addr = fabric_meta.spgw.s1u_sgw_addr;
        gtpu_ipv4.hdr_checksum = (bit<16>)16w0;
        gtpu_udp.setValid();
        gtpu_udp.sport = (bit<16>)16w2152;
        gtpu_udp.dport = (bit<16>)16w2152;
        gtpu_udp.len = fabric_meta.spgw.ipv4_len + 16w16;
        gtpu_udp.checksum = (bit<16>)16w0;
        gtpu.setValid();
        gtpu.version = (bit<3>)3w0x1;
        gtpu.pt = (bit<1>)1w0x1;
        gtpu.spare = (bit<1>)1w0;
        gtpu.ex_flag = (bit<1>)1w0;
        gtpu.seq_flag = (bit<1>)1w0;
        gtpu.npdu_flag = (bit<1>)1w0;
        gtpu.msgtype = (bit<8>)8w0xff;
...
parser FabricParser(packet_in packet, out parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    bit<6> last_ipv4_dscp = (bit<6>)6w0;
    state start {
    ...
    state parse_gtpu {
        transition select(hdr.ipv4.dst_addr[31:24]) {
            S1U_SGW_PREFIX[31:24]: do_parse_gtpu;
            default: accept;
        }
    }
    state do_parse_gtpu {

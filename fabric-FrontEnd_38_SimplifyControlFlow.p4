...
control Filtering(inout parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    ...
    @name("permit_with_internal_vlan") action permit_with_internal_vlan_0(vlan_id_t vlan_id) {
    ...
    @name("set_forwarding_type") action set_forwarding_type_0(fwd_type_t fwd_type) {
...
control Forwarding(inout parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    @hidden @name("set_next_id") action set_next_id_0(next_id_t next_id) {
        fabric_metadata.next_id = next_id;
    }
    @name("bridging_counter") direct_counter(CounterType.packets_and_bytes) bridging_counter_0;
    @name("set_next_id_bridging") action set_next_id_bridging_0(next_id_t next_id) {
        set_next_id_0(next_id);
    ...
    @name("pop_mpls_and_next") action pop_mpls_and_next_0(next_id_t next_id) {
    ...
    @name("set_next_id_routing_v4") action set_next_id_routing_v4_0(next_id_t next_id) {
        set_next_id_0(next_id);
...
control Acl(inout parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    ...
    @name("set_next_id_acl") action set_next_id_acl_0(next_id_t next_id) {
        fabric_metadata.next_id = next_id;
        acl_counter_0.count();
    }
...
control Next(inout parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    @hidden @name("output") action output_0(port_num_t port_num) {
        standard_metadata.egress_spec = port_num;
    }
    @hidden @name("rewrite_smac") action rewrite_smac_0(mac_addr_t smac) {
        hdr.ethernet.src_addr = smac;
    }
    @hidden @name("rewrite_dmac") action rewrite_dmac_0(mac_addr_t dmac) {
        hdr.ethernet.dst_addr = dmac;
    }
    @hidden @name("set_mpls_label") action set_mpls_label_0(mpls_label_t label) {
        fabric_metadata.mpls_label = label;
    }
    @hidden @name("routing") action routing_0(port_num_t port_num, mac_addr_t smac, mac_addr_t dmac) {
        rewrite_smac_0(smac);
        rewrite_dmac_0(dmac);
        output_0(port_num);
    }
    @hidden @name("mpls_routing") action mpls_routing_0(port_num_t port_num, mac_addr_t smac, mac_addr_t dmac, mpls_label_t label) {
        set_mpls_label_0(label);
        routing_0(port_num, smac, dmac);
    }
    @name("next_vlan_counter") direct_counter(CounterType.packets_and_bytes) next_vlan_counter_0;
    @name("set_vlan") action set_vlan_0(vlan_id_t vlan_id) {
        fabric_metadata.vlan_id = vlan_id;
    ...
    @name("output_xconnect") action output_xconnect_0(port_num_t port_num) {
        output_0(port_num);
        xconnect_counter_0.count();
    }
    @name("set_next_id_xconnect") action set_next_id_xconnect_0(next_id_t next_id) {
        fabric_metadata.next_id = next_id;
        xconnect_counter_0.count();
    }
    ...
    @name("output_hashed") action output_hashed_0(port_num_t port_num) {
        output_0(port_num);
        hashed_counter_0.count();
    }
    @name("routing_hashed") action routing_hashed_0(port_num_t port_num, mac_addr_t smac, mac_addr_t dmac) {
        routing_0(port_num, smac, dmac);
        hashed_counter_0.count();
    }
    @name("mpls_routing_hashed") action mpls_routing_hashed_0(port_num_t port_num, mac_addr_t smac, mac_addr_t dmac, mpls_label_t label) {
        mpls_routing_0(port_num, smac, dmac, label);
        hashed_counter_0.count();
    }
    ...
    @name("set_mcast_group_id") action set_mcast_group_id_0(mcast_group_id_t group_id) {
...
control spgw_ingress(inout ipv4_t gtpu_ipv4, inout udp_t gtpu_udp, inout gtpu_t gtpu, inout ipv4_t ipv4, inout udp_t udp, inout fabric_metadata_t fabric_meta, inout standard_metadata_t standard_metadata) {
    ...
    @name("set_dl_sess_info") action set_dl_sess_info_0(bit<32> teid, bit<32> s1u_enb_addr, bit<32> s1u_sgw_addr) {
...
control FabricComputeChecksum(inout parsed_headers_t hdr, inout fabric_metadata_t meta) {
    ...
    apply {
        ...
        update_gtpu_checksum_inst_0.apply(hdr.gtpu_ipv4, hdr.gtpu_udp, hdr.gtpu, hdr.ipv4, hdr.udp);
    }
}
...
parser FabricParser(packet_in packet, out parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    @name("last_ipv4_dscp") bit<6> last_ipv4_dscp_0;
    bit<4> tmp;
    bit<4> tmp_0;
    state start {
        last_ipv4_dscp_0 = 6w0;
        transition start_0;
    }
    state start_0 {
    ...
    state parse_ipv4 {
        ...
        last_ipv4_dscp_0 = hdr.ipv4.dscp;
    ...
    state do_parse_gtpu {
        ...
        last_ipv4_dscp_0 = hdr.inner_ipv4.dscp;
...
control FabricIngress(inout parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    ...
    apply {
        spgw_normalizer_inst_0.apply(hdr.gtpu.isValid(), hdr.gtpu_ipv4, hdr.gtpu_udp, hdr.ipv4, hdr.udp, hdr.inner_ipv4, hdr.inner_udp);
        pkt_io_ingress_0.apply(hdr, fabric_metadata, standard_metadata);
        filtering_0.apply(hdr, fabric_metadata, standard_metadata);
        spgw_ingress_inst_0.apply(hdr.gtpu_ipv4, hdr.gtpu_udp, hdr.gtpu, hdr.ipv4, hdr.udp, fabric_metadata, standard_metadata);
        if (fabric_metadata.skip_forwarding) {
            ;
        } else {
            forwarding_0.apply(hdr, fabric_metadata, standard_metadata);
        }
        acl_1.apply(hdr, fabric_metadata, standard_metadata);
        if (fabric_metadata.skip_next) {
            ;
        } else {
            next_0.apply(hdr, fabric_metadata, standard_metadata);
            port_counters_control_0.apply(hdr, fabric_metadata, standard_metadata);
        }
    }
}
...
control FabricEgress(inout parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    ...
    apply {
        ...
        egress_next_0.apply(hdr, fabric_metadata, standard_metadata);
        spgw_egress_inst_0.apply(hdr.ipv4, hdr.gtpu_ipv4, hdr.gtpu_udp, hdr.gtpu, fabric_metadata, standard_metadata);
    }
}

...
parser FabricParser(packet_in packet, out parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    @name("tmp") bit<4> tmp_1;
    @name("tmp_0") bit<4> tmp_2;
    state start {
        transition start_0;
    }
    state start_0 {
...
control FabricIngress(inout parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    @name("spgw_normalizer.hasReturned") bool spgw_normalizer_hasReturned_0;
    @name("spgw_ingress.hasReturned_0") bool spgw_ingress_hasReturned_0;
    @name(".nop") action nop() {
    }
    ...
    @name("spgw_ingress.ue_counter") direct_counter(CounterType.packets_and_bytes) spgw_ingress_ue_counter_0;
    @hidden @name("spgw_ingress.gtpu_decap") action spgw_ingress_gtpu_decap() {
        hdr.gtpu_ipv4.setInvalid();
        hdr.gtpu_udp.setInvalid();
        hdr.gtpu.setInvalid();
    }
    @name("spgw_ingress.set_dl_sess_info") action spgw_ingress_set_dl_sess_info(@name("teid") bit<32> teid_1, @name("s1u_enb_addr") bit<32> s1u_enb_addr_1, @name("s1u_sgw_addr") bit<32> s1u_sgw_addr_1) {
        ...
        spgw_ingress_ue_counter_0.count();
    }
    @name("spgw_ingress.dl_sess_lookup") table spgw_ingress_dl_sess_lookup_0 {
        key = {
        ...
        counters = spgw_ingress_ue_counter_0;
    }
    @name("spgw_ingress.s1u_filter_table") table spgw_ingress_s1u_filter_table_0 {
        key = {
    ...
    @name("filtering.ingress_port_vlan_counter") direct_counter(CounterType.packets_and_bytes) filtering_ingress_port_vlan_counter_0;
    @name("filtering.deny") action filtering_deny() {
        fabric_metadata.skip_forwarding = true;
        fabric_metadata.skip_next = true;
        filtering_ingress_port_vlan_counter_0.count();
    }
    @name("filtering.permit") action filtering_permit() {
        filtering_ingress_port_vlan_counter_0.count();
    }
    @name("filtering.permit_with_internal_vlan") action filtering_permit_with_internal_vlan(@name("vlan_id") vlan_id_t vlan_id_2) {
        fabric_metadata.vlan_id = vlan_id_2;
        filtering_ingress_port_vlan_counter_0.count();
    }
    @name("filtering.ingress_port_vlan") table filtering_ingress_port_vlan_0 {
        key = {
        ...
        counters = filtering_ingress_port_vlan_counter_0;
        size = 1024;
    }
    @name("filtering.fwd_classifier_counter") direct_counter(CounterType.packets_and_bytes) filtering_fwd_classifier_counter_0;
    @name("filtering.set_forwarding_type") action filtering_set_forwarding_type(@name("fwd_type") fwd_type_t fwd_type_1) {
        fabric_metadata.fwd_type = fwd_type_1;
        filtering_fwd_classifier_counter_0.count();
    }
    @name("filtering.fwd_classifier") table filtering_fwd_classifier_0 {
        key = {
            standard_metadata.ingress_port: exact @name("ig_port") ;
        ...
        counters = filtering_fwd_classifier_counter_0;
        size = 1024;
    }
    @name("forwarding.bridging_counter") direct_counter(CounterType.packets_and_bytes) forwarding_bridging_counter_0;
    @name("forwarding.set_next_id_bridging") action forwarding_set_next_id_bridging(@name("next_id") next_id_t next_id_0) {
        @hidden {
            fabric_metadata.next_id = next_id_0;
        }
        forwarding_bridging_counter_0.count();
    }
    @name("forwarding.bridging") table forwarding_bridging_0 {
        key = {
        ...
        counters = forwarding_bridging_counter_0;
        size = 1024;
    }
    @name("forwarding.mpls_counter") direct_counter(CounterType.packets_and_bytes) forwarding_mpls_counter_0;
    @name("forwarding.pop_mpls_and_next") action forwarding_pop_mpls_and_next(@name("next_id") next_id_t next_id_6) {
        fabric_metadata.mpls_label = 20w0;
        @hidden {
            fabric_metadata.next_id = next_id_6;
        }
        forwarding_mpls_counter_0.count();
    }
    @name("forwarding.mpls") table forwarding_mpls_0 {
        key = {
        ...
        counters = forwarding_mpls_counter_0;
        size = 1024;
    }
    @name("forwarding.routing_v4_counter") direct_counter(CounterType.packets_and_bytes) forwarding_routing_v4_counter_0;
    @name("forwarding.set_next_id_routing_v4") action forwarding_set_next_id_routing_v4(@name("next_id") next_id_t next_id_7) {
        @hidden {
            fabric_metadata.next_id = next_id_7;
        }
        forwarding_routing_v4_counter_0.count();
    }
    @name("forwarding.nop_routing_v4") action forwarding_nop_routing_v4() {
        forwarding_routing_v4_counter_0.count();
    }
    @name("forwarding.routing_v4") table forwarding_routing_v4_0 {
        key = {
        ...
        counters = forwarding_routing_v4_counter_0;
        size = 1024;
    }
    @name("acl.acl_counter") direct_counter(CounterType.packets_and_bytes) acl_acl_counter_0;
    @name("acl.set_next_id_acl") action acl_set_next_id_acl(@name("next_id") next_id_t next_id_8) {
        fabric_metadata.next_id = next_id_8;
        acl_acl_counter_0.count();
    }
    ...
    @name("acl.acl") table acl_acl_0 {
        ...
        size = 1024;
        counters = acl_acl_counter_0;
    }
    @name("next.next_vlan_counter") direct_counter(CounterType.packets_and_bytes) next_next_vlan_counter_0;
    @name("next.set_vlan") action next_set_vlan(@name("vlan_id") vlan_id_t vlan_id_3) {
        fabric_metadata.vlan_id = vlan_id_3;
        next_next_vlan_counter_0.count();
    }
    @name("next.next_vlan") table next_next_vlan_0 {
        key = {
        ...
        counters = next_next_vlan_counter_0;
        size = 1024;
    }
    @name("next.xconnect_counter") direct_counter(CounterType.packets_and_bytes) next_xconnect_counter_0;
    @name("next.output_xconnect") action next_output_xconnect(@name("port_num") port_num_t port_num) {
        @hidden {
            standard_metadata.egress_spec = port_num;
        }
        next_xconnect_counter_0.count();
    }
    @name("next.set_next_id_xconnect") action next_set_next_id_xconnect(@name("next_id") next_id_t next_id_9) {
        fabric_metadata.next_id = next_id_9;
        next_xconnect_counter_0.count();
    }
    @name("next.xconnect") table next_xconnect_0 {
        key = {
            standard_metadata.ingress_port: exact @name("ig_port") ;
            fabric_metadata.next_id       : exact @name("next_id") ;
        }
        ...
        counters = next_xconnect_counter_0;
        const default_action = nop_15();
        size = 1024;
    }
    @max_group_size(16) @name("next.hashed_selector") action_selector(HashAlgorithm.crc16, 32w1024, 32w16) next_hashed_selector_0;
    @name("next.hashed_counter") direct_counter(CounterType.packets_and_bytes) next_hashed_counter_0;
    @name("next.output_hashed") action next_output_hashed(@name("port_num") port_num_t port_num_0) {
        @hidden {
            standard_metadata.egress_spec = port_num_0;
        }
        next_hashed_counter_0.count();
    }
    @name("next.routing_hashed") action next_routing_hashed(@name("port_num") port_num_t port_num_1, @name("smac") mac_addr_t smac, @name("dmac") mac_addr_t dmac) {
        @hidden {
            @hidden {
    ...
    @name("next.mpls_routing_hashed") action next_mpls_routing_hashed(@name("port_num") port_num_t port_num_2, @name("smac") mac_addr_t smac_0, @name("dmac") mac_addr_t dmac_0, @name("label") mpls_label_t label_0) {
        @hidden {
            @hidden {
    ...
    @name("next.hashed") table next_hashed_0 {
        key = {
            fabric_metadata.next_id : exact @name("next_id") ;
        ...
        implementation = next_hashed_selector_0;
        counters = next_hashed_counter_0;
        const default_action = nop_16();
        size = 1024;
    }
    @name("next.multicast_counter") direct_counter(CounterType.packets_and_bytes) next_multicast_counter_0;
    @name("next.set_mcast_group_id") action next_set_mcast_group_id(@name("group_id") mcast_group_id_t group_id) {
        standard_metadata.mcast_grp = group_id;
        fabric_metadata.is_multicast = true;
        next_multicast_counter_0.count();
    }
    @name("next.multicast") table next_multicast_0 {
        key = {
            fabric_metadata.next_id: exact @name("next_id") ;
        }
        actions = {
        ...
        counters = next_multicast_counter_0;
        const default_action = nop_17();
        size = 1024;
    }
    @name("port_counters_control.egress_port_counter") counter(32w511, CounterType.packets_and_bytes) port_counters_control_egress_port_counter_0;
    @name("port_counters_control.ingress_port_counter") counter(32w511, CounterType.packets_and_bytes) port_counters_control_ingress_port_counter_0;
    apply {
        hdr.gtpu_ipv4.setInvalid();
        hdr.gtpu_udp.setInvalid();
        spgw_normalizer_hasReturned_0 = false;
        if (hdr.gtpu.isValid()) {
            ;
        } else {
            spgw_normalizer_hasReturned_0 = true;
        }
        if (spgw_normalizer_hasReturned_0) {
            ;
        } else {
        ...
        filtering_ingress_port_vlan_0.apply();
        filtering_fwd_classifier_0.apply();
        spgw_ingress_hasReturned_0 = false;
        if (hdr.gtpu.isValid()) {
            if (spgw_ingress_s1u_filter_table_0.apply().hit) {
                ;
            } else {
                mark_to_drop(standard_metadata);
            }
        ...
        } else if (spgw_ingress_dl_sess_lookup_0.apply().hit) {
            fabric_metadata.spgw.direction = 2w2;
        } else {
            fabric_metadata.spgw.direction = 2w0;
            spgw_ingress_hasReturned_0 = true;
        }
        if (spgw_ingress_hasReturned_0) {
            ;
        } else {
        ...
        if (fabric_metadata.skip_forwarding) {
            ...
            forwarding_bridging_0.apply();
        } else if (fabric_metadata.fwd_type == 3w1) {
            forwarding_mpls_0.apply();
        } else if (fabric_metadata.fwd_type == 3w2) {
            forwarding_routing_v4_0.apply();
        }
        acl_acl_0.apply();
        if (fabric_metadata.skip_next) {
            ;
        } else {
            next_xconnect_0.apply();
            next_hashed_0.apply();
            next_multicast_0.apply();
            next_next_vlan_0.apply();
            if (standard_metadata.egress_spec < 9w511) {
                port_counters_control_egress_port_counter_0.count((bit<32>)standard_metadata.egress_spec);
            }
            if (standard_metadata.ingress_port < 9w511) {
                port_counters_control_ingress_port_counter_0.count((bit<32>)standard_metadata.ingress_port);
            }
        }
    }
}
...
control FabricEgress(inout parsed_headers_t hdr, inout fabric_metadata_t fabric_metadata, inout standard_metadata_t standard_metadata) {
    ...
    @name("egress_next.egress_vlan_counter") direct_counter(CounterType.packets_and_bytes) egress_next_egress_vlan_counter_0;
    @name("egress_next.pop_vlan") action egress_next_pop_vlan() {
        hdr.ethernet.eth_type = fabric_metadata.eth_type;
        hdr.vlan_tag.setInvalid();
        egress_next_egress_vlan_counter_0.count();
    }
    @name("egress_next.egress_vlan") table egress_next_egress_vlan_0 {
        key = {
            fabric_metadata.vlan_id      : exact @name("vlan_id") ;
        ...
        counters = egress_next_egress_vlan_counter_0;
        size = 1024;
    }
    apply {
        ...
        if (egress_next_egress_vlan_0.apply().hit) {
            ;

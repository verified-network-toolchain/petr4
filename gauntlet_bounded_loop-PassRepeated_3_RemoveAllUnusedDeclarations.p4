...
parser p(packet_in packet, out headers hdr) {
    @name("sub_parser.tracker") bit<8> sub_parser_tracker_0;
    state start {
        hdr.nop.setInvalid();
        hdr.p.setInvalid();
    ...
    state sub_parser_start {
        sub_parser_tracker_0 = 8w0;
        transition sub_parser_next;
    }
    state sub_parser_next {
        transition select(sub_parser_tracker_0 == 8w1) {
    ...
    state sub_parser_parse_hdr {
        sub_parser_tracker_0 = sub_parser_tracker_0 + 8w1;

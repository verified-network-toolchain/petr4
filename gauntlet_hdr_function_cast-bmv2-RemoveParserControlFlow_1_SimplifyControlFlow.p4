...
control ingress(inout Headers h, inout Meta m, inout standard_metadata_t sm) {
    apply {
        {
            bit<16> val_0 = h.eth_hdr1.eth_type;
            @name("hasReturned") bool hasReturned_1;
            @name("retval") ethernet_t retval_1;
            hasReturned_1 = false;
            if (val_0 == 16w1) {
                hasReturned_1 = true;
                retval_1.setValid();
                retval_1 = (ethernet_t){dst_addr = 48w1,src_addr = 48w1,eth_type = 16w1};
            } else if (val_0 == 16w2) {
                hasReturned_1 = true;
                retval_1.setValid();
                retval_1 = (ethernet_t){dst_addr = 48w2,src_addr = 48w2,eth_type = 16w2};
            }
            if (hasReturned_1) {
                ;
            } else {
                hasReturned_1 = true;
                retval_1.setValid();
                retval_1 = (ethernet_t){dst_addr = 48w3,src_addr = 48w3,eth_type = 16w3};
            }
        ...
        }
        {
            ...
            @name("retval_0") ethernet_t retval_2;
            hasReturned_2 = false;
            hasReturned_2 = true;
            retval_2.setValid();
            retval_2 = (ethernet_t){dst_addr = 48w1,src_addr = 48w1,eth_type = 16w1};
            h.eth_hdr2 = retval_2;
        }
    }
}

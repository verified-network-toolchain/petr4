...
control egress(inout Headers h) {
    @name("ipv4_checksum") Checksum() ipv4_checksum;
    apply {
        h.h.result = ipv4_checksum.update<tuple<bit<48>, bit<48>, bit<16>>>({ h.eth_hdr.dst_addr, h.eth_hdr.src_addr, h.eth_hdr.eth_type });
    }
}

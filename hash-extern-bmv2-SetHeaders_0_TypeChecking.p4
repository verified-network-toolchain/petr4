...
control ingress(inout headers_t hdr, inout metadata_t user_meta, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    ...
    @noWarnUnused @name(".send_to_port") action send_to_port() {
        meta_1 = ostd;
        egress_port_1 = istd.ingress_port;
        meta_1.drop = false;
...
control egress(inout headers_t hdr, inout metadata_t user_meta, in psa_egress_input_metadata_t istd, inout psa_egress_output_metadata_t ostd) {
    @name("base") bit<4> base_1;
    @name("max") bit<4> max_1;
    @name("h") Hash<bit<16>>(PSA_HashAlgorithm_t.CRC16) h_2;
    apply {
        base_1 = 4w0x6;
        max_1 = 4w0xa;
        hdr.ipv4.hdrChecksum = h_2.get_hash<bit<4>, tuple<bit<4>, bit<4>, bit<8>, bit<16>, bit<16>, bit<3>, bit<13>, bit<8>, bit<8>, bit<32>, bit<32>>>(base_1, { hdr.ipv4.version, hdr.ipv4.ihl, hdr.ipv4.diffserv, hdr.ipv4.totalLen, hdr.ipv4.identification, hdr.ipv4.flags, hdr.ipv4.fragOffset, hdr.ipv4.ttl, hdr.ipv4.protocol, hdr.ipv4.srcAddr, hdr.ipv4.dstAddr }, max_1);
    }
}

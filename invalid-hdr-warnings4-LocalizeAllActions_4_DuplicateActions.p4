...
control IngressI(inout H hdr, inout M meta, inout standard_metadata_t smeta) {
    @name("h") H h_0;
    @name("h_copy") H h_copy_0;
    @name("h_copy2") H h_copy2_0;
    @name("i") bit<1> i_0;
    bit<1> tmp;
    @name("validateHeader") action validateHeader_0(@name("hd") inout Header hd_0) {
        hd_0.setValid();
    }
    @name("validateHeader") action validateHeader(@name("hd") inout Header hd_0) {
        hd_0.setValid();
    }
    @name("validateHeader") action validateHeader_1(@name("hd") inout Header hd_0) {
        hd_0.setValid();
    }
    @name("invalidateHeader") action invalidateHeader_0(@name("hd") inout Header hd_1) {
        hd_1.setInvalid();
    }
    @name("invalidateHeader") action invalidateHeader(@name("hd") inout Header hd_1) {
        hd_1.setInvalid();
    }
    @name("invalidateStack") action invalidateStack_0(@name("stack") inout Header[2] stack_0) {
        {
            @name("hd") Header hd = stack_0[0];
            hd.setInvalid();
            stack_0[0] = hd;
        }
        {
            @name("hd") Header hd_2 = stack_0[1];
            hd_2.setInvalid();
            stack_0[1] = hd_2;
        }
    }
    @name("invalidateStack") action invalidateStack(@name("stack") inout Header[2] stack_0) {
        {
            @name("hd") Header hd = stack_0[0];
            hd.setInvalid();
            stack_0[0] = hd;
    ...
    @name("invalidateStack") action invalidateStack_1(@name("stack") inout Header[2] stack_0) {
        {
            @name("hd") Header hd = stack_0[0];
            hd.setInvalid();
            stack_0[0] = hd;
    ...
    apply {
        ...
        invalidateHeader(h_0.h2[0]);
        i_0 = 1w1;
        tmp = i_0;
        validateHeader_1(h_0.h2[tmp]);
        invalidateStack(h_0.h1);
        invalidateStack_1(h_copy_0.h1);
        h_0.h1[1].setValid();
    }
}

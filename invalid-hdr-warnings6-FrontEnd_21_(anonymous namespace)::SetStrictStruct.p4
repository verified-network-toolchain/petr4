...
parser ParserI(packet_in pkt, out H hdr, inout M meta, inout standard_metadata_t smeta) {
    state start {
        ...
        hdr.u[0].h1.data = (bit<32>)32w1;
        hdr.u[0].h3.data = (bit<32>)32w1;
...
control IngressI(inout H hdr, inout M meta, inout standard_metadata_t smeta) {
    ...
    apply {
        ...
        u[1].h1 = u[0].h1;
        u[1].h1.data = (bit<32>)32w1;
        bit<1> i = (bit<1>)1w0;
        u[0].h2.setValid();
        u[1] = u[i];
        u[1].h1.data = (bit<32>)32w1;
        u[1].h2.data = (bit<16>)16w1;
        if (u[1].h2.data == 16w0) {
            u[i].h2.setValid();
        }
        u[0].h1.data = (bit<32>)32w1;
        if (u[1].h2.data == 16w0) {
            u[i].h1.setValid();
        } else {
            u[i].h2.setValid();

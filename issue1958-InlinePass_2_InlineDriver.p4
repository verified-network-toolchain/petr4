...
control foo2(inout headers_t my_headers, inout metadata_t meta, register<bit<8>> my_reg) {
    @name("idx") bit<32> idx_0;
    @name("val") bit<8> val_0;
    @name("foo2_action") action foo2_action_0() {
        idx_0 = (bit<32>)my_headers.ethernet.srcAddr[7:0];
        my_reg.read(val_0, idx_0);
        val_0 = val_0 + 8w249;
        my_reg.write(idx_0, val_0);
    }
    @name("foo2_table") table foo2_table_0 {
        key = {
...
control ingressImpl(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t stdmeta) {
    @name("reg1") register<bit<8>>(32w256) reg1_0;
    @name("foo2_inst") foo2() foo2_inst_0;
    @name("foo2_inst.idx") bit<32> foo2_inst_idx;
    @name("foo2_inst.val") bit<8> foo2_inst_val;
    @name("foo2_inst.foo2_action") action foo2_inst_foo2_action() {
    ...
    @name("foo2_inst.foo2_table") table foo2_inst_foo2_table {
        key = {
            hdr.ethernet.srcAddr: exact @name("my_headers.ethernet.srcAddr") ;
        }
        actions = {
            foo2_inst_foo2_action();
            NoAction();
        }
        default_action = NoAction();
    }

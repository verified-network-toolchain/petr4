...
control Eg(inout Headers hdrs, inout Metadata meta, inout standard_metadata_t standard_meta) {
    @name("inKey") Key inKey;
    @name("defaultKey") Key defaultKey;
    @name("same") bool same;
    @name("val") Value val_1;
    @name("done") bool done;
    @name("ok") bool ok;
    @name("test") action test_0() {
        inKey = (Key){field1 = 32w1};
        defaultKey = (Key){field1 = 32w0};
        same = inKey == defaultKey;
        val_1 = (Value){field1 = 32w0};
        done = false;
        ok = !done && same;
        if (ok) {
            @name("val") Value val_2 = val_1;
            val_2.field1 = 32w8;
            val_1 = val_2;
        }
    }

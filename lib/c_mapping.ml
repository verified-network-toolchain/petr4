...
and Parameter : sig
  ...
  val format_params_semi : P4.Parameter.t list -> _ Pp.t
...
and Declaration : sig
  ...
  and format_t e =
    ...
    | Control { annotations; name; type_params; params; constructor_params; locals; apply } ->
      let n = text (snd name) ++ (text "_state") in 
      ...
      ++ box ~indent:2 (text "void " ++ text (snd name) ++ text "(" ++ n ++ text "* state) {" ++ newline ++
                        (match (snd name) with 
                         | "C" -> box ~indent:2 (text "if(state->hdrs.simple.dst != 0) {" ++ newline ++
                                                 text "C_forward(state);") ++ newline ++ 
                                  box ~indent:2 (text "} else { " ++ newline ++ 
                                                 text "C_drop(state);") ++ newline ++ 
                                  text "}"
    ...
    | Instantiation { annotations; typ; args; name; init=None } ->
      box ~indent: 2 (text "int main() {" ++ newline ++
                      text "P_state p;" ++ newline ++
                      text "C_state c;" ++ newline ++
                      text "D_state d;" ++ newline ++
                      text "p.pkt = 'Hi';" ++ newline ++
                      text "P(&p);" ++ newline ++
                      text "c.hdrs = p.hdrs;" ++ newline ++
                      text "C(&c);" ++ newline ++
                      text "d.hdrs = c.hdrs;" ++ newline ++
                      text "d.pkt = malloc(2);" ++ newline ++
                      text "D(&d);" ++ newline ++
                      text "return 0;") ++ newline 
      ++text "}"
    ...
    | ValueSet { annotations; typ; size; name } ->
      ...
      (box ~indent:2 (("value_set<" |> text) ++ (Type.format_t typ) ++
                      ...
                      (";" |> text)))
    | TypeDef { annotations; name; typ_or_decl } -> nop

 ...
 * TypeName references.
 *)
let rec saturate_type (env: Env.CheckerEnv.t) (typ: Type.t) : Type.t =
  let open Type in
  ...
  match typ with
  | TypeName typ ->
      saturate_type env (Env.CheckerEnv.resolve_type_name typ env)
  | Bool | String | Integer
...
and type_equality' (env: Env.CheckerEnv.t)
...
let assert_same_type (env: Env.CheckerEnv.t) info1 info2 (typ1: Type.t) (typ2: Type.t) =
...
let compile_time_known_expr (_: Env.CheckerEnv.t) (_: Expression.t) : unit =
  failwith "Unimplemented"

let rec type_expression_dir (env: Env.CheckerEnv.t) ((_, exp): Expression.t) : Type.t * direction =
  let open Env.CheckerEnv in
  match exp with
  | True ->
  ...
  | Name name ->
    find_type_of (snd name) env
  | TopLevel name ->
    find_type_of_toplevel (snd name) env
...
and type_expression (env: Env.CheckerEnv.t) (exp : Expression.t) : Type.t =
  fst (type_expression_dir env exp)

and translate_type (env: Env.CheckerEnv.t) (typ: Types.Type.t) : Typed.Type.t =
...
and type_error_member env name : Typed.Type.t =
  let (e, _) = Env.CheckerEnv.find_type_of ("error." ^ (snd name)) env in
  match e with
...
and type_expression_member env expr name : Typed.Type.t =
  ...
  let fields =
    ...
    | TypeName na ->
      begin match Env.CheckerEnv.resolve_type_name na env with
...
and check_call env func type_args args post_check : 'a =
  ...
  let get_fun_type = fun (ft:Types.Expression.t) ->
    begin match snd ft with
      | Name na ->
        begin match resolve_type_name (snd na) env with
        | Type.Function fr -> fr
  ...
  let extend_delta environ (t_par, t_arg) =
    insert_type t_par t_arg environ
  in
...
and type_function_call env func type_args args =
  ...
  let post_check fun_type =
    ...
    (* helper to extend delta environment *)
    let extend_delta environ (t_par, t_arg) =
      Env.CheckerEnv.insert_type t_par t_arg environ
    in
...
and type_statement (env: Env.CheckerEnv.t) (stm: Statement.t) : (StmType.t * Env.CheckerEnv.t) =
  match snd stm with
...
and type_method_call env func type_args args =
  ...
  let post_check = fun ft ->
    ...
    let pfold = fun (acc:Env.CheckerEnv.t) (p:Parameter.t) ->
      match p.direction with
      | In -> acc
      | Directionless | Out | InOut -> (* only out variables are added to the environment *)
          Env.CheckerEnv.insert_type_of p.name p.typ env
    in
...
and type_declaration (env: Env.CheckerEnv.t) ((_, decl): Declaration.t) : Env.CheckerEnv.t =
  match decl with
...
and type_constant env typ name value =
  ...
  Env.CheckerEnv.insert_dir_type_of (snd name) expr_typ In env
...
and type_function env return name type_params params body =
  let p_fold = fun (acc,env:Parameter.t list * Env.CheckerEnv.t) (p:Types.Parameter.t) ->
    begin let open Parameter in
    let p = snd p in
      let pd = begin match p.direction with
      | None -> In
    ...
    let par = {name=p.variable |> snd;
              ...
              direction=pd} in
    let new_env =
      Env.CheckerEnv.insert_dir_type_of (snd p.variable) par.typ par.direction env
    in
    par::acc, new_env end in
  let (ps,body_env) = List.fold_left p_fold ([],env) params in
  let ps = List.rev ps in
  let rt = return |> translate_type env  in
  let sfold = fun (prev_type,envi:StmType.t*Env.CheckerEnv.t) (stmt:Statement.t) ->
  ...
  let funtype = Type.Function {parameters=ps;
                 ...
                 return= rt} in
  Env.CheckerEnv.insert_type_of (snd name) funtype env
...
and type_variable env typ name init =
  ...
  match init with
  | None ->
      Env.CheckerEnv.insert_dir_type_of (snd name) expected_typ In env
...
and type_header env name fields =
  ...
  Env.CheckerEnv.insert_type (snd name) header_typ env
...
and type_header_union env name fields =
  ...
  let union_folder = fun acc -> fun field ->
    begin let open Types.Declaration in
      ...
      match Env.CheckerEnv.resolve_type_name ht env with
  ...
  Env.CheckerEnv.insert_type (snd name) hu env

(* Section 7.2.5 *)
and type_struct env name fields =
  let fields = List.map (type_field env) fields in
  let struct_typ = Type.Struct { fields } in
  Env.CheckerEnv.insert_type (snd name) struct_typ env
...
and type_error env members =
  let add_err env (_, e) =
    Env.CheckerEnv.insert_type_of_toplevel ("error." ^ e) Type.Error env
  in
...
and type_match_kind env members =
  let add_mk env (_, m) =
    Env.CheckerEnv.insert_type_of_toplevel ("match_kind." ^ m) Type.MatchKind env
  in
...
and type_enum env name members =
  ...
  Env.CheckerEnv.insert_type (snd name) enum_typ env
...
and type_control_type env name type_params params =
  ...
  Env.CheckerEnv.insert_type (snd name) ctrl env
...
let check_program (program:Types.program) : Env.CheckerEnv.t =
  let top_decls = match program with Program tds -> tds in
  let init_acc = Env.CheckerEnv.empty_checker_env in

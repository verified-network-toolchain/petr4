...
module PreUp4Filter : Target = struct
  ...
  (* Return length of the packet_in's size *)
  let eval_get_length : extern = fun env st ts args ->
    let loc = match args with
      | [(VRuntime {loc = l; _}, _)] -> l
      | _ -> failwith "unexpected args for get_length" in
    let pck_obj = State.find_extern loc st in
  ...
  let eval_set_out_port : extern = fun env st ts args -> 
    ...
    let in_port, queue_depth_at_dequeue = match im_t_obj with 
      | Im_t {in_port = i; queue_depth_at_dequeue = q; _} -> i, q
      | _ -> failwith "cannot perform set_out_port on a non-im_t object" in
  ...
  let eval_get_in_port : extern = fun env st ts args -> 
    ...
    let in_port, queue_depth_at_dequeue = match im_t_obj with
      | Im_t {in_port = i; queue_depth_at_dequeue = q; _} -> i, q 
      | _ -> failwith "cannot perform get_in_port on a non-im_t object" in 
  ...
  let eval_get_out_port : extern = fun env st ts args -> 
    ...
    let out_port = match im_t_obj with
      | Im_t {out_port = o; _} -> o 
      | _ -> failwith "cannot perform get_out_port on a non-im_t object" in 
  ...
  let eval_get_value : extern = fun env st ts args -> 
    ...
    let v = match enum_name, im_t_obj with
      ...
      | "QUEUE_DEPTH_AT_DEQUEUE", _ -> failwith "cannot perform get_value on a non-im_t object" 
  ...
  let eval_drop : extern = fun env st ts args -> 
    ...
    let in_port, queue_depth_at_dequeue = match im_t_obj with 
      | Im_t {in_port = i; queue_depth_at_dequeue = q; _} -> i, q
      | _ -> failwith "cannot perform drop on a non-im_t object" in
  ...
  (* [copy_from] extern function of pkt and im_t externs are not supported *)
  ...
  let initialize_metadata meta st =
    st |> State.insert_extern im_t_location (
          Im_t {out_port = meta; in_port = meta; queue_depth_at_dequeue = Bigint.of_int 32}) 
       |> State.insert_extern pkt_location Pkt
  ...
  let eval_pipeline (ctrl : ctrl) (env : env) (st : state) (pkt : pkt)
    ...
    let vpkt, pkt_name, vpkt_loc = VRuntime {loc = State.packet_location; obj_name = "packet_in"; }, 
                                   ...
                                   State.fresh_loc() in 
    let p, p_name, p_loc = VRuntime {loc = pkt_location; obj_name = "pkt"; }, 
                           Types.BareName (Info.dummy, "p"),
                           State.fresh_loc() in
    ...
    let st = st
             ...
             |> State.insert_heap p_loc p  
             |> State.insert_heap im_loc im in
    let env =
      EvalEnv.(env
               |> insert_val pkt_name vpkt_loc
               |> insert_val p_name p_loc
               |> insert_val im_name  im_loc
               |> insert_typ pkt_name (List.nth_exn params 0).typ
               |> insert_typ p_name  (List.nth_exn params 1).typ 
               |> insert_typ im_name  (List.nth_exn params 2).typ) in
    let hdrs, hdrs_name, hdrs_loc, st, env = 
      helper "hdrs" (List.nth_exn params 3).typ env st in
    let meta, meta_name, meta_loc, st, env = 
      helper "meta" (List.nth_exn params 4).typ env st in
    let in_p, in_p_name, in_p_loc, st, env = 
      helper "in_param" (List.nth_exn params 5).typ env st in
    let inout_p, inout_p_name, inout_p_loc, st, env = 
      helper "inout_param" (List.nth_exn params 6).typ env st in
    let out_p, out_p_name, out_p_loc, st, env = 
      helper "out_param" (List.nth_exn control_params 5).typ env st in
    let open Expression in
    let pkt_expr = 
      Some (Info.dummy, {expr = Name pkt_name; dir = Directionless; typ = (List.nth_exn params 0).typ}) in
    let p_expr = 
      Some (Info.dummy, {expr = Name p_name; dir = Directionless; typ = (List.nth_exn params 1).typ}) in
    let im_expr = 
      Some (Info.dummy, {expr = Name im_name; dir = Directionless; typ = (List.nth_exn params 2).typ}) in
    let hdrs_expr =
      Some (Info.dummy, {expr = Name hdrs_name; dir = Out; typ = (List.nth_exn params 3).typ}) in
    let meta_expr =
      Some (Info.dummy, {expr = Name meta_name; dir = InOut; typ = (List.nth_exn params 4).typ}) in
    let in_expr =
      Some (Info.dummy, {expr = Name in_p_name; dir = In; typ = (List.nth_exn params 5).typ}) in
    let inout_expr =
      Some (Info.dummy, {expr = Name inout_p_name; dir = InOut; typ = (List.nth_exn params 6).typ}) in
    let out_expr = 
      Some (Info.dummy, {expr = Name out_p_name; dir = Out; typ = (List.nth_exn control_params 5).typ}) in
    (* Go through micro parser *)
    let (st,signal, _) =
      app ctrl env st SContinue parser [pkt_expr; p_expr; im_expr; hdrs_expr; meta_expr; in_expr; inout_expr] in
    match signal with 
    | SReject _ -> st, env, None
    | SContinue | SExit | SReturn _ -> 
      (* Go through micro control *)
      let (st, signal) = eval_up4_ctrl ctrl control 
          [p_expr; im_expr; hdrs_expr; meta_expr; 
      ...
      | SContinue | SExit | SReturn _ -> 
        ...
        let outport = match im_t with 
          | Im_t {out_port; _} -> out_port 
          | _ -> failwith "im_t_location has a non-im_t object" in 
  ...
  let get_outport (st : state) (env : env) : Bigint.t =
    ...
    | _ -> failwith "im_t_location has a non-im_t object"

...
let assert_P4core = function
  | P4coreObject o -> o
  | V1Object _ -> failwith "expected P4core object"

let is_P4core = function
  | P4coreObject _ -> true
  | V1Object _ -> false
...
let corize_st (st : state) : P4core.state =
  st
  |> List.filter ~f:(fun (_, o) -> is_P4core o)
  |> List.map ~f:(fun (i, o) -> (i, assert_P4core o))

let targetize_st (st : P4core.state) : state = 
  st
  |> List.map ~f:(fun (i, o) -> i, P4coreObject o)
...
let eval_pipeline
  ...
  let st = State.insert pkt_loc (P4coreObject (P4core.make_pkt_in pkt)) st in
  let hdr =
  ...
  let st = 
    State.insert 
      pktout_loc 
      (P4coreObject (P4core.make_pkt_out (Cstruct.create 0) (State.find pkt_loc st
                                                             |> assert_P4core |> P4core.assert_pkt_in))) st in
  ...
  | VRuntime {loc; _ } -> 
     let (p0, p1) = State.find loc st |> assert_P4core |> P4core.assert_pkt_out in
     st, env, Cstruct.append p0 p1

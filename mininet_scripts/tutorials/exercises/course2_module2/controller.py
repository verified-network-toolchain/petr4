...
class MyApp(App):
  def init_topo(self):
    ...
    topo.add_link("h3", "s11", 0, 3, 1)
    topo.add_link("h4", "s12", 0, 3, 1)
    ...
    topo.add_link("s6", "s8", 4, 1, 1)
    topo.add_link("s6", "s9", 3, 1, 1)
    topo.add_link("s6", "s11", 5, 4, 1)

    topo.add_link("s7", "s9", 3, 2, 1)
    topo.add_link("s7", "s10", 4, 3, 1)
    topo.add_link("s7", "s12", 5, 4, 1)
    
    topo.add_link("s8", "s9", 2, 6, 1)
    topo.add_link("s8", "s11", 3, 2, 1)
 
    topo.add_link("s9", "s10", 3, 1, 1)
    topo.add_link("s9", "s11", 5, 1, 1)
    topo.add_link("s9", "s12", 4, 2, 1)
  ...
  def optimize_paths(self): 
    ...
    for d in self.demands:
      ...
      while not done:
        ...
        for x in nei:
          ...
          if var_name in solver.vars:
            val = solver.vars[var_name].value()
            if val > 0:
              non_zero.append(x)
    ...
    for l in self.topo.links():
      a, b = l
      if a in self.topo.hosts() or b in self.topo.hosts():
        continue
      s = 0
      ds = []
      for d in self.demands:

...
control Eg(inout Headers hdrs, inout Metadata meta, inout standard_metadata_t standard_meta) {
    @name("_sub") bit<32> _sub_0;
    @name("res") bit<64> res_0;
    bit<32> tmp;
    @name("update") action update_0(@name("p") in bool p_0, @name("val") inout bit<64> val_0) {
        _sub_0 = val_0[31:0];
        if (p_0) {
            tmp = _sub_0;
        } else {
    ...
    @name("update") action update(@name("p") in bool p_0, @name("val") inout bit<64> val_0) {
        _sub_0 = val_0[31:0];
        if (p_0) {
            tmp = _sub_0;
        } else {

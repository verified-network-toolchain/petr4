...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    apply {
        if (hdr.h2.isValid()) {
            if (hdr.h4.isValid()) {
                standard_metadata.egress_spec = (bit<9>)9w2;
            } else {
                standard_metadata.egress_spec = (bit<9>)9w12;
            }
        } else if (hdr.h3.isValid()) {
            if (hdr.h4.isValid()) {
                standard_metadata.egress_spec = (bit<9>)9w3;
            } else {
                standard_metadata.egress_spec = (bit<9>)9w13;
            }
        } else if (hdr.h4.isValid()) {
            standard_metadata.egress_spec = (bit<9>)9w11;
        } else {
            standard_metadata.egress_spec = (bit<9>)9w10;
        }
    }
}

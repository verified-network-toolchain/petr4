...
control MainControlImpl(inout headers_t hdr, inout main_metadata_t user_meta, in pna_main_input_metadata_t istd, inout pna_main_output_metadata_t ostd) {
    @name("forward") action forward_0(@name("addr") bit<32> addr_0) {
        user_meta.meta = addr_0;
    }
    @name("forward") action forward(@name("addr") bit<32> addr_0) {
        user_meta.meta = addr_0;
    }
    @name("forward") action forward_1(@name("addr") bit<32> addr_0) {
        user_meta.meta = addr_0;
    }
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            forward_1(SelectByDirection<bit<32>>(istd.direction, hdr.ipv4.srcAddr, hdr.ipv4.dstAddr));

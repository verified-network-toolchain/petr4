...
control ingress(inout Headers h, inout Meta m, inout standard_metadata_t sm) {
    ...
    @name(".assign") action assign(@name("eth_t") inout bit<16> eth_t, @name("target_addr") inout bit<48> target_addr, @name("check_bool") bool check_bool) {
        if (check_bool) {
            tmp_1 = simple_check();
            if (tmp_1) {
                tmp_2 = 16w0xdead != eth_t;
            } else {
                tmp_2 = false;
            }
            if (tmp_2) {

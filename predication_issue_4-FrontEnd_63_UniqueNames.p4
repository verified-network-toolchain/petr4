...
control ingress(inout Headers h, inout Meta m, inout standard_metadata_t sm) {
    @name("bool_val") bool bool_val_0;
    @name("tmp_0") bit<1> tmp;
    @name("tmp_1") bit<1> tmp_0;
    @name("perform_action") action perform_action() {
        if (bool_val_0) {
            {
                @name("val_0") bit<1> val = 1w0;
                @name("bound_0") bit<1> bound = 1w1;

...
@noWarnUnused @name(".egress_drop") action egress_drop(@name("meta") inout psa_egress_output_metadata_t meta_1) {
    meta_1.drop = true;
}
...
control cIngress(inout headers_t hdr, inout metadata_t user_meta, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    @noWarnUnused @name(".send_to_port") action send_to_port_0(@name("meta") inout psa_ingress_output_metadata_t meta_0, @name("egress_port") in PortId_t egress_port_0) {
        meta_0.drop = false;
        meta_0.multicast_group = (MulticastGroup_t)32w0;
        meta_0.egress_port = egress_port_0;
    }
    ...
    @noWarnUnused @name(".send_to_port") action send_to_port_2(@name("meta") inout psa_ingress_output_metadata_t meta_0, @name("egress_port") in PortId_t egress_port_0) {
        meta_0.drop = false;
        meta_0.multicast_group = (MulticastGroup_t)32w0;
        meta_0.egress_port = egress_port_0;
    }
    apply {
        if (hdr.ethernet.dstAddr == 48w8 && istd.packet_path != PSA_PacketPath_t.RECIRCULATE) {
            send_to_port_1(ostd, (PortId_t)32w0xfffffffa);
        } else {
            send_to_port_2(ostd, (PortId_t)(PortIdUint_t)hdr.ethernet.dstAddr);
        }
    }
}
...
control cEgress(inout headers_t hdr, inout metadata_t user_meta, in psa_egress_input_metadata_t istd, inout psa_egress_output_metadata_t ostd) {
    @noWarnUnused @name(".egress_drop") action egress_drop_0(@name("meta") inout psa_egress_output_metadata_t meta_1) {
        meta_1.drop = true;
    }
    @noWarnUnused @name(".egress_drop") action egress_drop_1(@name("meta") inout psa_egress_output_metadata_t meta_1) {
        meta_1.drop = true;
    }
    @name("clone") action clone_0() {
        ostd.clone = true;
        ostd.clone_session_id = (CloneSessionId_t)16w8;
    }
    @name("clone") action clone_1() {
        ostd.clone = true;
        ostd.clone_session_id = (CloneSessionId_t)16w8;
    }
    apply {

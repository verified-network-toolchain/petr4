...
control cIngress(inout headers_t hdr, inout metadata_t user_meta, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    ...
    apply {
        ...
        packet_path_to_int_inst_1.apply(istd.packet_path, hdr.output_data.word0);
    }
}
...
control cEgress(inout headers_t hdr, inout metadata_t user_meta, in psa_egress_input_metadata_t istd, inout psa_egress_output_metadata_t ostd) {
    ...
    @name("egress_pkt_seen") Register<bit<16>, bit<8>>(32w256) egress_pkt_seen_0;
    apply {
        @name("idx") bit<8> idx_0 = hdr.ethernet.etherType[7:0];
        @name("cur_count") bit<16> cur_count_0 = egress_pkt_seen_0.read(idx_0);
        if (hdr.ethernet.etherType[15:8] == 8w0xc0) {
            hdr.output_data.word0 = (bit<32>)cur_count_0;
        } else if (hdr.ethernet.etherType[15:8] == 8w0xc1) {
            @name("write_data") bit<16> write_data_0 = hdr.ethernet.srcAddr[15:0];

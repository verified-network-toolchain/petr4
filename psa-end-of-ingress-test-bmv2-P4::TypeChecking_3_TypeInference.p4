...
control cEgress(inout headers_t hdr, inout metadata_t user_meta, in psa_egress_input_metadata_t istd, inout psa_egress_output_metadata_t ostd) {
    @name("idx") bit<8> idx;
    @name("cur_count") bit<16> cur_count;
    @name("write_data") bit<16> write_data;
    @name("egress_pkt_seen") Register<bit<16>, bit<8>>(32w256) egress_pkt_seen;
    apply {
        idx = hdr.ethernet.etherType[7:0];
        cur_count = egress_pkt_seen.read(idx);
        if (hdr.ethernet.etherType[15:8] == 8w0xc0) {
            hdr.output_data.word0 = (bit<32>)cur_count;
        } else if (hdr.ethernet.etherType[15:8] == 8w0xc1) {
            write_data = hdr.ethernet.srcAddr[15:0];

...
control ingress(inout headers hdr, inout metadata_t user_meta, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    ...
    @name("color_in") PSA_MeterColor_t color_in_0 = PSA_MeterColor_t.RED;
    @name("execute") action execute_0(bit<12> index) {
        color_out_0 = meter0_0.execute(index, color_in_0, hdr.ipv4.totalLen);
        user_meta.port_out = (color_out_0 == PSA_MeterColor_t.GREEN ? 32w1 : 32w0);
        reg_0.write(index, user_meta.port_out);
    }

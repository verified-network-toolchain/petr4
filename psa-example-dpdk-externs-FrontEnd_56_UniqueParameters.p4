...
control ingress(inout headers hdr, inout metadata_t user_meta, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    ...
    @name("color_out") PSA_MeterColor_t color_out;
    @name("color_in") PSA_MeterColor_t color_in;
    @name("tmp") bit<32> tmp_0;
    @name("counter0") Counter<bit<10>, bit<12>>(32w1024, PSA_CounterType_t.PACKETS_AND_BYTES) counter0;
    @name("counter1") Counter<bit<10>, bit<12>>(32w1024, PSA_CounterType_t.PACKETS) counter1;
    @name("counter2") Counter<bit<10>, bit<12>>(32w1024, PSA_CounterType_t.BYTES) counter2;
    @name("reg") Register<bit<32>, bit<12>>(32w1024) reg;
    @name("meter0") Meter<bit<12>>(32w1024, PSA_MeterType_t.BYTES) meter0;
    @name("execute") action execute_0(@name("index") bit<12> index_1) {
        color_out = meter0.execute(index_1, color_in, hdr.ipv4.totalLen);
        if (color_out == PSA_MeterColor_t.GREEN) {
            tmp_0 = 32w1;
        } else {
            tmp_0 = 32w0;
        }
        user_meta.port_out = tmp_0;
        reg.write(index_1, user_meta.port_out);
    }
    @name("tbl") table tbl {
        key = {

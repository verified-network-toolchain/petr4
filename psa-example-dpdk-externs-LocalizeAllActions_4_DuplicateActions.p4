...
control ingress(inout headers hdr, inout metadata_t user_meta, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    ...
    @name("color_out") PSA_MeterColor_t color_out_0;
    @name("color_in") PSA_MeterColor_t color_in_0;
    bit<32> tmp;
    @name("counter0") Counter<bit<10>, bit<12>>(32w1024, PSA_CounterType_t.PACKETS_AND_BYTES) counter0_0;
    @name("counter1") Counter<bit<10>, bit<12>>(32w1024, PSA_CounterType_t.PACKETS) counter1_0;
    @name("counter2") Counter<bit<10>, bit<12>>(32w1024, PSA_CounterType_t.BYTES) counter2_0;
    @name("reg") Register<bit<32>, bit<12>>(32w1024) reg_0;
    @name("meter0") Meter<bit<12>>(32w1024, PSA_MeterType_t.BYTES) meter0_0;
    @name("execute") action execute_0(@name("index") bit<12> index_0) {
        color_out_0 = meter0_0.execute(index_0, color_in_0, hdr.ipv4.totalLen);
        ...
        reg_0.write(index_0, user_meta.port_out);
    }
    @name("execute") action execute_1(@name("index") bit<12> index_0) {
        color_out_0 = meter0_0.execute(index_0, color_in_0, hdr.ipv4.totalLen);
        if (color_out_0 == PSA_MeterColor_t.GREEN) {
            tmp = 32w1;
        } else {

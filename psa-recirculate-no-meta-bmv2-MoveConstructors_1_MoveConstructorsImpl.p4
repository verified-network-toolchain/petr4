...
@noWarnUnused action send_to_port(@name("meta") inout psa_ingress_output_metadata_t meta_0, @name("egress_port") in PortId_t egress_port_0) {
    meta_0.drop = false;
...
control cIngress(inout headers_t hdr, inout metadata_t user_meta, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    @name("int_packet_path") bit<32> int_packet_path_0;
    @name("packet_path_to_int") packet_path_to_int() packet_path_to_int_inst_1;
    @name("record_ingress_ports_in_pkt") action record_ingress_ports_in_pkt_0() {
        hdr.output_data.word1 = (PortIdUint_t)istd.ingress_port;
    }
    apply {
        if (hdr.ethernet.dstAddr[3:0] >= 4w4) {
            record_ingress_ports_in_pkt_0();
            send_to_port(ostd, (PortId_t)(PortIdUint_t)hdr.ethernet.dstAddr);
        } else {
            send_to_port(ostd, (PortId_t)32w0xfffffffa);
        }
        packet_path_to_int_inst_1.apply(istd.packet_path, int_packet_path_0);
...
control cEgress(inout headers_t hdr, inout metadata_t user_meta, in psa_egress_input_metadata_t istd, inout psa_egress_output_metadata_t ostd) {
    @name("packet_path_to_int") packet_path_to_int() packet_path_to_int_inst_2;
    ...
    apply {
        ...
        if (istd.egress_port == (PortId_t)32w0xfffffffa) {
            packet_path_to_int_inst_2.apply(istd.packet_path, hdr.output_data.word3);
        }
    }
}

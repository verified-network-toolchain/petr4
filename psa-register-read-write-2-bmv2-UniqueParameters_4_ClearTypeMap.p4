...
control MyIC(inout headers_t hdr, inout metadata_t user_meta, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    ...
    @name("idx") bit<8> idx;
    @name("action_type") bit<8> action_type;
    @name("orig_data") bit<16> orig_data;
    @name("next_data") bit<16> next_data;
    @name("validAction") bool validAction;
    @name("reg") Register<bit<16>, bit<8>>(32w6) reg;
    apply {
        if (hdr.ethernet.isValid()) {
            idx = hdr.ethernet.dstAddr[7:0];
            action_type = hdr.ethernet.dstAddr[15:8];
            validAction = action_type >= 8w1 && action_type <= 8w3;
            if (validAction) {

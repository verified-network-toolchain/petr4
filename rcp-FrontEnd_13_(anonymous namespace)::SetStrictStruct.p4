...
control ingress(inout H pkt_hdr, in Metadata metadata) {
    ...
    apply {
        @atomic {
            ...
            input_traffic_bytes.read((bit<32>)0, input_traffic_bytes_tmp);
            input_traffic_bytes_tmp = input_traffic_bytes_tmp + metadata.pkt_len;
            input_traffic_bytes.write(input_traffic_bytes_tmp, (bit<32>)0);
            if (pkt_hdr.rtt < 32w2500) {
                bit<32> sum_rtt_Tr_tmp;
                sum_rtt_Tr.read((bit<32>)0, sum_rtt_Tr_tmp);
                sum_rtt_Tr_tmp = sum_rtt_Tr_tmp + pkt_hdr.rtt;
                sum_rtt_Tr.write(sum_rtt_Tr_tmp, (bit<32>)0);
                bit<32> num_pkts_with_rtt_tmp;
                num_pkts_with_rtt.read((bit<32>)0, num_pkts_with_rtt_tmp);
                num_pkts_with_rtt_tmp = num_pkts_with_rtt_tmp + 32w1;
                num_pkts_with_rtt.write(num_pkts_with_rtt_tmp, (bit<32>)0);
            }
        }
    }
}

...
control Ingress(inout headers_t hdr, inout meta_t meta) {
    @name("hash1") Hash<bit<32>>(algo = HashAlgorithm_t.CRC32) hash1;
    @name("tmp") CRCPolynomial<bit<16>>(coeff = 16w0x107) tmp_0;
    @name("hash2") Hash<bit<32>>(poly = tmp_0) hash2;
    @name("hamming") LCGMatrix<bit<8>, bit<10>>(8w4, 10w7, "1000100
         0100011
         0010110
         0001001") hamming;
    @name("hash3") Hash<bit<32>>(matrix = hamming) hash3;
    @name("hash4") Hash<bit<32>>(formula = "magic_formula()") hash4;
    apply {
    }
}

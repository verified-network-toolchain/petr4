...
control ingressImpl(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t stdmeta) {
    ...
    @name("redirect") action redirect_0() {
        stdmeta.egress_spec = stdmeta.ingress_port;
    }
    @name("redirect") action redirect() {
        stdmeta.egress_spec = stdmeta.ingress_port;
    }
    @MixedExprList[1, "hello", true, false, 11] @MixedKV[label="text", my_bool=true, int_val=6] @name("t") table t_0 {
        key = {
        }
        actions = {
            @myid[1] redirect();

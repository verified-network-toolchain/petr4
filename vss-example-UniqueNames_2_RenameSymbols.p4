...
control TopPipe(inout Parsed_packet headers, in error parseError, in InControl inCtrl, out OutControl outCtrl) {
    ...
    @name("Set_nhop") action Set_nhop(@name("ipv4_dest") IPv4Address ipv4_dest, @name("port") PortId port) {
        nextHop_0 = ipv4_dest;
    ...
    @name("Set_dmac") action Set_dmac(@name("dmac") EthernetAddress dmac_0) {
    ...
    @name("Set_smac") action Set_smac(@name("smac") EthernetAddress smac_0) {
